...
- **声明式装配（Phase 6 + 组合架构 S1 三层，2026-08-20；P4 B①/②/A-1/B④ 修订 2026-08-23）**：内置工具族由 `src/composition/tool-rows.ts` 行表装配（9 行内置族，factory → Tool[]，行内重名装载期拒绝；git/search/fs/shell/agent-isolation 五族已迁 ctx.tools 第一方插件通道）；system-prompt 段落由 `src/composition/prompt-sections.ts` 单一真源定义（13 段，两装配面 applicable 分流；P4 B④ 收官起全量经 `plugins/prompt-segments-plugin.ts` 走 ctx.prompts 通道贡献，出厂段表退役；插件段贡献追加在解析产物末尾，下次装配生效）；会话级工具/hook 仍由 `agent/blueprint.ts` 的 `AgentBlueprint` capability 表驱动——**`AgentConfig` 冻结 31 字段**不再扩张；三层表序 = 字节契约（DeepSeek 前缀缓存与 effective 快照依赖此序）；teardown 走 `ctx.effect`；面板/命令/工具/provider 四 service 注册表挂根 Context（`src/composition/services.ts`，`ContributionRegistry` 内核：装载期重名拒绝 + disposer 双守卫）+ 块渲染器第五（`renderer-service.tsx`）+ prompt 段第六（`prompt-service.ts`）
