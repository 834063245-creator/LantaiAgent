...
- **八条贡献通道**：`ctx.panels` / `ctx.commands` / `ctx.tools` / `ctx.llm`（LLM adapter）/ `ctx.renderers`（纸壳块体）/ `ctx.prompts`（system-prompt 段）/ `ctx.hooks`（工具管道钩子：enrich 富化 / preflight 预检）/ `ctx.capabilities`（会话级能力）——service 注册表挂根 Context（`composition/services.ts` 等：装载期重名拒绝 + disposer 双守卫）
