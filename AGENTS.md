...
- 第一方插件清单身份（2026-08-29）：`plugins/first-party-manifest.ts` 是 43 个第一方插件的身份单一真源（name → version/description/kind；`service` = 平台服务常驻不可禁，`feature` = 功能插件可禁用）。设置面板「插件」tab 三组陈列：平台服务 / 内置插件 / 已安装（第三方）。feature 类启用/禁用经 `state/plugin-prefs.ts`（localStorage 持久化）**下次启动生效**（loader boot 跳过被禁插件）；装载结果统一收 `state/plugin-store.ts`（builtin + meta，`mergePlugins` 按 name 合并——第一方 boot 与第三方异步装载互不冲刷）。**新增第一方插件 = `BUILTIN_PLUGINS` 加行 + 本清单加条目**（守护 `tests/first-party-manifest.test.ts`，漏条目 = loader 跳过 + error 记录）。
