...
>
> **用户允许 Agent 对代码库进行重构、清理、架构整理等破坏性操作，一切由测试工程兜底。
> Agent 发现不合理的代码、架构、行为、参数、存储等问题，可进行大刀阔斧的修改，以结果论对错。**
>
> 背景：LLM 的天性是保守兼容（往上堆、兼容旧有行为），本项目的历史包袱正是「破坏性操作干得不彻底」留下的（铺卷化石等三轮「重构」都绕着旧时序走）。所以：**发现旧的不合理行为，默认拆除而非兼容；兼容层只在用户明确要求时建；拆干净比绕着走重要；测试全绿就是验收标准。**「不迁移、直接归档」「双读回退整链删除」这类操作在本仓库是常态而非例外。
