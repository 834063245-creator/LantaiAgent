...
| 前端 | `cd src-ui && npx vitest run` | 300 文件 3025 passed / 4 skipped（2026-09-14 修复批实测；**convergence 默认门禁 = standard 单轨**——`verify:convergence` 与 CI 都不设 `CONVERGENCE_PRESET`，minimal 轨须显式运行且其快照当前已漂移（见本节尾注）；本机注意：父进程带 `NODE_ENV=production` 会使 convergence specs 收集阶段报 `No such built-in module: node:` 并剥 devDependencies——跑测试前清掉该变量） |
...
> ⚠ **convergence 双轨现状（2026-09-14 审计实测，纠正本节与 `CLAUDE.md` 此前的「双 preset 零漂移」表述）**：`npm run verify:convergence` 与 CI（`.github/workflows/convergence.yml`）都**只跑 standard 轨**——两者都不设 `CONVERGENCE_PRESET`，`gate.mjs` 缺省即 standard。实测 `CONVERGENCE_PRESET=minimal npx vitest run tests/convergence/specs/phase-0.test.ts` **当前为红**（tool-schemas.full 期望 count 12 / 实际 13，plan 14/15）：`2d54081f`（office 域改一等工具）只重录了 standard 快照，`baseline/preset-minimal/` 停留在 2026-09-09。同款漏项在 `docs/archive/agent-core-convergence/baseline-change-request-tool-ergonomics-waist.md:40-48` 已记过一次（原话「minimal 套件不在默认门禁（需 CONVERGENCE_PRESET=minimal 显式运行），漏项潜伏两天未被发现」，同款先例 5b5c20a9）——**本次是第三次复发**。**待办（均需人类审批）**：① 走 `baseline-change-request` 重录 minimal 快照（record 只允许出现在审批提交里）；② 决定是否给 `verify:convergence`/CI 补第二条轨，否则该轨会继续静默腐烂。
