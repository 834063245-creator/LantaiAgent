...
| 前端 | `cd src-ui && npx vitest run` | 161 文件 1598 passed / 1 skipped（2026-08-23 S4-4 甲实测；convergence 双 preset 零漂移；graph-engine-toggle 在并行窗口活跃期有已知假红，单跑即绿；本机注意：父进程带 `NODE_ENV=production` 会使 convergence specs 收集阶段报 `No such built-in module: node:` 并剥 devDependencies——跑测试前清掉该变量） |
