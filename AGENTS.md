...
| 前端 | `cd src-ui && npx vitest run` | 157 文件 1557 passed / 1 skipped（2026-08-23 P4 B① 实测，+1 文件 +8 用例 = git-search-plugin 通道钉；convergence 双 preset 零漂移；本机注意：父进程带 `NODE_ENV=production` 会使 convergence specs 收集阶段报 `No such built-in module: node:` 并剥 devDependencies——跑测试前清掉该变量） |
