...
- **卷首组合芯片**：每卷卷首显示本卷组合名（空白卷可拨 = 写本卷卷级选择；跑过一轮 = 只读标签）；读写判据全经 core 能力位（`sessionComposition` / `isSessionBlank` / `selectSessionPreset`）。**落位纪律**：绝对定位覆盖、**不进高度流水**（卷首高度是几何契约 `FOLIO_TOKENS` → `measureFolioHeadHeight`）；卷首本体 `pointer-events: none`，只放开本子树。细则与同屏并排证据见 `docs/plans/composition-architecture/designs/S6-per-agent-composition.md` §8。
