...
- **第一方插件清单**（2026-08-29）：`plugins/first-party-manifest.ts` = 43 个第一方插件身份单一真源（`service` 平台服务常驻不可禁 / `feature` 功能插件可禁用）；设置面板「插件」tab 三组陈列（平台服务/内置插件/已安装）；feature 启用/禁用经 `state/plugin-prefs.ts`（localStorage）**下次启动生效**；装载统一收 `state/plugin-store.ts`（builtin+meta，mergePlugins 按 name 合并）。**新增第一方插件 = BUILTIN_PLUGINS 加行 + 清单加条目**（守护 `tests/first-party-manifest.test.ts` 钉死）。
