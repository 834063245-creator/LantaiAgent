...
禁止用 `cargo build --release` 代替桌面发布验证。当前实测基线：engine 697 tests（696 passed / 1 ignored）· src-tauri bin 389 + 集成 14（全绿；2026-08-22 第 5 棒实测，含 rpc Value 化第二步；cdp e2e 偶现 ±1）· 前端 158 文件 1566 passed / 1 skipped（2026-08-23 P4 B①+A-1 实测；convergence 双 preset 零漂移；本机跑测试前清 `NODE_ENV=production`，否则 specs 收集报 `No such built-in module: node:`）。
