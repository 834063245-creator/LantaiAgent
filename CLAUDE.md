...
- **token 计量（2026-09-13 立账）**：真源 = `agent/token-meter/`（纯层：分桶代数 `usage.ts` / 构成测量 `estimate.ts` / 每卷账本 `SessionTokenMeter`）+ Agent 侧每卷一本账（`getTokenStats / snapshotTokenLedger / restoreTokenLedger`，句柄上的**能力位**——句柄不实现 = 无读数，不炸链路）。录入点唯一 = `Agent.streamOnce`（每请求一条记录：构成 + 用量）；投递面 = `LoopStreamResult.token` → `EventKind.Usage` sink → `chat-stream` 的 `_recordTokens` → 卷级 `sessionTokens`（总量投影 + 重算触发）；落盘 = 卷文件 `tokens` 字段（旧卷无此字段 = 从空开始）。口径纪律（沿 DSH token-meter，禁漂移）：输入四桶互不重叠且加总恒等于提供方 `prompt_tokens`（残差法归一）；压力只算 prompt 侧（不含输出）；占用 = 投影（压力 + 采样后载荷增量，夹零）；构成是估算不是账单（与压缩预检共用 `token-counter.ts` 同一把尺子）；缓存命中率部分命中绝不四舍五入成 100%。UI 面 = 创作坞「墨量册」（`plugins/builtin/compose-dock/InkLedger.tsx`——坞底墨量线的展开账，契约见 `docs/design/lantai-design-spec.md` §9.1）。`agent-loop/types.ts` / `default-loop.ts` 是开放面契约文件——改它们按 `composition/contract-version.ts` 四步流程升版。
...
- **工具**：模型工具必须 `defineTool` + zod v4；领域动作变更同步 `DOMAIN_SPECS` / 测试（`collectHiddenToolNames()` 已派生自 `DOMAIN_SPECS`，2026-09-14 起不必手工登记）。禁止手写 schema、execute 里 `as` 强拆、用 `.strict()`。
- **Agent 运行时**（组合架构 S1 三层 + S2 外化 + S4 preset realm/热重载/安装通道，2026-08-20 起）：内置工具族（①c 后仅 web/browser-desktop 2 行）加行到 `src/composition/tool-rows.ts` 行表；第一方工具域插件（B①/② 无状态五族 + ①c 装配期真值七族 wait/ask/memory/skill/task/agent/hologram）经 `plugins/builtin/<domain>/` + `composition/first-party-tools.ts` 清单走 ctx.tools 贡献（无状态族实例缓存；真值族贡献 noCache 每装配重创）；system-prompt 段落定义留 `src/composition/prompt-sections.ts` 单一真源（分隔符是字节契约禁规整；P4 B④ 收官起 13 段全量经 `plugins/builtin/prompt-segments/` 走 ctx.prompts 通道贡献，出厂段表退役）；插件 prompt 段贡献（P4 A-1）经 `ctx.prompts`（`composition/prompt-service.ts`）下次装配生效；S4-4 甲（2026-08-23）组合解析域收编通道贡献——factoryComposition() 快照 pluginToolRows 行 + prompt 段贡献，patch/preset 可寻址 plugin/<贡献 id> 行与贡献段 id（含 13 第一方段）；**平台化 P3（2026-08-27）seam 裁剪域**——factoryComposition() 收编六条 `seam/<域>` 寻址域（llm/subagents/fs/shell/sessionPersistence/loopEvents——`graph` 域随图谱功能全量退役，2026-09-09），patch/preset 可禁用 provider、开关 emit 观测事件（消费视图 = 活动注册表 − 禁用集，过滤收在 active* 消费单点与 emitLoopEvent；**S6 P2（2026-09-15）起裁剪面按 Agent 取值**——有组合上下文的路径传该组合的 `seamDisabled`（fs/shell 工具族按 executor 注入的 `_owner_id` 查装配期登记表 `composition/seam-scope.ts`；subagents 用本 Agent 组合；`emitLoopEvent` 读本 Agent 总线 `setSeamView` 灌入的视图；llm 经 `createProvider` 的 `options.seamView`），无组合上下文的旧路径仍读全局当前选择（零漂移），`seam/sessionPersistence` 例外仍全局（per-volume 后端需另立「卷→组合」外部索引）；契约版本现 **v39**）；**平台化 P4（2026-08-27）运行时插件全链路**——D6 外部插件装/卸/启用/禁用**运行时生效**（loader 活跃注册表 + activate/deactivateExternalPlugin，fiber dispose 链式回收）；D7 `ctx.dynamicRunner`（agent/dynamic-runner/）——模型经 **cordis 域**（define/run/stop/undefine/inspect，形状对齐 DSH tool-cordis）运行时定义插件包：approval 门（首激活 UI 批准）+ 宿主半沙箱（危险全局阴影 / 守卫注册面 / 预算三层防线）+ 包不可变/失败回滚/会话所有权隔离；进程外能力面收口 = `examples/plugins/dataflow-mcp/`（外部 MCP server 端到端例子，`./` 前缀 args 相对插件目录解析）；信任模型见 `docs/plugins/README.md` §6；**平台化 P5（2026-08-28）存量迁移**——D13 `ctx.agentLoop`（agent/agent-loop/：AgentLoop/AgentLoopHost + builtin/default + 注册表后注册胜）流式循环降为第一方默认实现（行为逐字节一致）；工具管道生产路径切 eventBus（构造期 attachPlanGate + setters 各自 attach；差分 trace 钉住等价）；第一方 loop 可观测监听器 observability.ts；P5-C1/C2 守卫测试 first-party-surface + agent-loop-seam；**平台化 P6（2026-08-28）平台税收口**——插件面人类契约 = `docs/plugins/README.md`（§0 平台契约总览）；各 seam cookbook = `docs/cookbook/`（llm/subagents/fs/shell/session 后端 + 动态插件 + MCP server 同 AGENTS.md）；三方发布路径 = `docs/user/develop/publishing-plugins.md`；跨 seam 替换集成 = `tests/cross-seam-swap.test.ts`；会话级工具/hook 加项到 `agent/blueprint.ts` capability 表，不改 `AgentConfig`（冻结 **28** 字段——`tests/convergence/gate.mjs` 断言 `=== 28`；本文旧记「31 字段」是历史漂移，2026-09-15 核准更正）。三层表序 = 字节契约（前缀缓存 + effective 快照依赖）；teardown 走 `ctx.effect`，不做进 capability。session 变异只走 `_appendMessage` / `_replaceSession` / `_retractSessionRange` 三入口；改工具折叠同步 `session-log.ts` 的 `derivePayload`。**S2 起**：用户层 patch（`~/.lantai/composition/roster.patch.yml` → `composition/roster.ts` `resolveRoster`）可禁用/覆盖/插入四域行（语法见 `docs/composition/README.md`；寻址域 = builtin 行表 + 通道贡献快照（S4-4 甲：plugin 贡献行/段可寻址禁用/覆盖/锚定））；10 壳行（`composition/shell-rows.ts` + `src/shell/rows/*` + `src/shell/boot.ts`）承载引导职责——新引导接线加壳行，不往 main.ts 堆。**S4 起**：四 service 消费闭环已接线（面板 `panelDefs()` 合流 / 命令面板折算 / 插件工具行 `plugin/<贡献 id>` 折算进 buildToolRegistry）；**V3b 起**：第五贡献通道 `ctx.renderers` 块渲染器（`composition/renderer-service.tsx`——纸壳块体渲染经 `resolveRenderer(kind)` 解析，后注册胜 + `*` 兜底）+ 纸面板迁 PanelsService 贡献（`paper/paper-plugin.ts`，panel-def 常量面已迁出 paper 行）；preset realm（`composition/presets.ts` 内置表 + `preset-discovery.ts` 用户目录 + `preset-assembly.ts` cache/选择同步；层序 factory → 用户层 → preset；装配面可选 composition 覆盖参数）；热重载（composition_watcher → `composition:changed` → `reloadCompositionPatch`）；插件安装通道（`plugin_install`/`plugin_uninstall`/`plugin_set_enabled` + `plugin_dir`（S4-4 乙机器桥的目录锚点） + 设置「插件」tab；manifest `mcpServers` 声明式挂接外部 MCP server（S4-4 乙：一 server 一条贡献行 plugin/<插件名>/mcp/<server名>，lazy/startup-error 失败策略，kill 挂插件 fiber disposer）；插件自包含无裸 import——宿主桥 `window.__lantai_plugin_host__`，范本 `examples/plugins/hello/`，契约 `docs/plugins/README.md`）。**S3 起（2026-08-22）**：第一方域行化先例——settings 域经 `plugins/settings-plugin.ts` 双贡献（面板 + 命令）、paper 域补齐 `paper/toggle`；`PANEL_DEFS` 常量面清空（全量面板走贡献）；快捷键链路经 `app/actions.ts` 别名翻译层（`ACTION_CONTRIBUTION_ALIASES`）桥接，useGlobalKeys 字面量不变。**P4 起（2026-08-23）**：第六贡献通道 `ctx.prompts`（`composition/prompt-service.ts`）——插件注系统提示段落，`assembleSystemPrompt` 末端追加。以上全部门禁化：`npm run verify:convergence` 失败即返工（standard preset 快照零漂移）。
- **插件激活（S6 P3，2026-09-15 三笔落地；施工单 `docs/plans/composition-architecture/work-orders/WO-S6P3-plugin-activation.md`）**：**登记 ≠ 激活**——manifest 可选块 `activation: { lazy?, resources?, exclusive? }`；声明 `lazy: true` 的插件在 apply 里只 `ctx.activation.declare(插件名, { start, stop })` 登记，副作用启动挪到**组合装配期**（`_assembleAgent` retain，引用计数；Agent dispose / 切组合 → 归零 stop，句柄走 `AgentContext.effect` 对称释放）。账本体 = 叶模块 `composition/activation.ts`（零项目内运行时 import，环守卫钉住）；服务面 = `composition/activation-service.ts` 的**第五个组合层 service**（挂既有 `hologram/composition-services`，不新增插件条目 ⇒ 计数不动）。**kill switch**：不写 `activation` 块 ⇒ 新行为不发生（出厂 43 插件零声明 ⇒ 双轨零漂移是构造性结论）。**组合层新增 `requires` / `exclusive` 两个 patch 顶层键**（用户 preset 可写）：`requires` 缺插件 → 选择期拒 + 具名原因、解析期 F1 捕获网回退（不抛）；`exclusive` 与插件 `spec.exclusive` 同等参与装配期冲突检测（后装配者被拒 + 整体回滚）。诊断面扩**四栏**（第四栏「被跳过」= 激活失败插件 + 原因，设置面板「组合」节呈现）。性能对表见 `docs/plans/composition-architecture/reports/perf-after-S6P3.md`（零回归）。
- **程序入口（S6 P4，2026-09-16 落地；施工单 `docs/plans/composition-architecture/work-orders/WO-S6P4-program-entry.md`）**：`app/chat/session-composition.ts` 的 **`createSessionWithPreset(ctx, presetId?)`** = UI 之外的**按组合起卷**单点（评测自举 / 未来外部协议驱动 / 插件编排共用）——显式参数在 `createNewSession` **发号之后、调工厂之前**落卷级登记（`CreateSessionOptions.presetId`）⇒ 该卷**出生即按该组合装配一次**（不白装配、不写全局设置）；优先级链 = 显式参数 > 卷级记录 > 全局默认；**只收 preset id**（组合必须有限可枚举，内联 patch 不入 API）；不可解析（不在册 / `requires` 缺插件 / 行 id 不可寻址）⇒ **拒绝创建 + 具名原因、一个卷都不建**（严一档 `sessionSelectionError`，与 P1c 同尺）。`createNewSession` 返回值改 `number \| null`（消掉「静默失败后读到旧活跃卷 id」的陷阱）。**形态边界（实测裁定）**：不新增 Rust 命令（平台边界；会话属能力契约层 seam）· 不加模型可见工具参数（两轨 `tool-schemas` 快照漂 ⇒ baseline CR，且与 S4-1a 父子同面冲突）· 本批不接外部协议（兰台不是 MCP server、ACP server 零 boot 接线）⇒ **契约仍 v39**、不触发 baseline CR。**「起卷时指定」与「空白卷上拨组合」是两条语义**，共用同一把尺子与同一份记录真源。
- **卷首组合芯片（S6 P5，2026-09-16 两笔 `87681434` P5a + 收官写回；施工单 `docs/plans/composition-architecture/work-orders/WO-S6P5-ui-face.md`）**：**每个卷的卷首右上角显示该卷的组合名**（`plugins/builtin/paper-shell/FolioCompositionChip.tsx`）——空白卷可拨（写**本卷**的卷级选择，对非活跃卷同样生效）、跑过一轮 = 只读标签；hover 显示来源（本卷记录 / 全局默认）与不可用原因。读面/判据/写面全经 core 能力位（`sessionComposition` / `isSessionBlank` / `selectSessionPreset`，与创作坞芯片同一套）；卷首侧只补 `usePresetStore` 一个 host 出口（faceDeps 已含 ⇒ 不动 `host-modules.ts` 与 `host-surface.baseline.json`）。**落位纪律**：绝对定位覆盖、**不进高度流水**（卷首高度是几何契约 `FOLIO_TOKENS` → `measureFolioHeadHeight` → 卷级几何）；卷首本体仍 `pointer-events: none`，只放开本子树。**同屏并排两 Agent = 同纸多卷**（本批只补 chip 与证据，`tests/paper-side-by-side-agents.test.tsx`）；真面板并排（多 ChatCore 七面）与跨工作区并存留独立批次（设计件 §8.5 事实 3）。契约仍 v39。
- **第一方插件清单**（2026-08-29 立账；2026-09-03 S5 竣工；2026-09-14 计数核准 43 个 = 13 内核 + 30 出厂产物（旧记 44/14 是漂移，多算 1））：`plugins/first-party-manifest.ts` = 第一方插件身份单一真源（kind 降级为展示分组标签——`service` 内核不可禁 / `feature` 出厂产物可禁用）；设置面板「插件」tab 三组陈列（平台服务/内置插件/已安装）；feature 启用/禁用经 `state/plugin-prefs.ts`（localStorage）**下次启动生效**；装载统一收 `state/plugin-store.ts`。**S5（2026-09-03）：BUILTIN_PLUGINS 只装 13 内核（SERVICE_META 手写 13 条）；30 出厂产物真源在 `plugins/builtin/<name>/`（磁盘通道装载，改插件 = 换产物不重编译 exe）；dev 模式经 `plugins/factory-products.ts` 的 import.meta.env.DEV 分支走源码路径；装载调度 = cordis fiber PENDING + `plugins/boot-gate.ts` 全 ACTIVE 审计 fail-loud**。**新增出厂产物 = ①产品目录建 index.ts ② builtin-roster.json 加条目（唯一真源；产物 manifest.json 由构建脚本生成，不手写）③ 插件对象进出厂装配面（直接 import 的产物在 factory-products.ts 加行；工具/prompt/capability 域产物只进各自通道清单）；构建脚本与 Rust 资产通道都不用动（插件规格已从名册派生、plugin_assets.rs 无名字白名单）；first-party-manifest.ts 是派生面不需手工加条目**（守护 `tests/first-party-manifest.test.ts` + `tests/builtin-roster.test.ts`）。
