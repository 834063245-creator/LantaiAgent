...
impl Graph {
    ...
    /// 邻居查询（BFS 按深度扩散）。返回 (当前节点, 邻居, 耦合深度)。
    pub fn neighbors(&self, node_id: &str, depth: usize) -> Vec<(String, String, u8)> {
    ...
    /// 按名称/ID 子串搜索节点。
    pub fn search_nodes<'a>(&'a self, query: &str) -> Vec<&'a Node> {
...
mod tests {
    ...
    #[test]
    fn test_neighbors() {
        let g = test_query_graph();
        let nb = g.neighbors("b", 1);
