...
2026-08-25 增补（`docs/plans/agent-platformization-plan.md` Phase 0）。软件以平台为方向：能拆的都要拆成行 / provider，特权区只减不增。
...
- 权威扩展：强制层/能力契约层清单与施工顺序以 `docs/plans/agent-platformization-plan.md` 为准，本条约不自行演化。
