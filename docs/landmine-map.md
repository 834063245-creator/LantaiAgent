...
| L3 | `provider/idle-stream.ts` + `agent/agent.ts:stream()` | 无「请求硬截止」：全链路唯一的活性守卫只会 abort fetch | 卡在非 fetch 的 await（本机 IPC / 凭据解析 / 任何不理会 signal 的等待）时，30s 守卫是**空操作** → 回合永久挂起：无错误、无重试、无日志、无 UI 反馈，UI 永卡运行态直到重启 | ⏳ 未立项（②.2）。**可执行复现（2026-09-20 补）**：`tests/landmine-l3-hang.test.ts`（两条 `it.fails`——断的是「修好后应有的行为」，今天必然失败；L3 一拆它们转红，届时应改写为正向断言）。机制现状（三层守卫，只有一层管模型请求链）：① `idle-stream` 30s 只 abort 一个 controller——兜得住「fetch 认 abort」（→ 停滞错误 → `STALL_RETRY_BUDGET_MS` 15 分钟预算内重试 → 可见失败），兜不住不认 abort 的 await（计时器 abort 的对象没人听，`for await` 永不返回 ⇒ 重试循环无从前进 ⇒ 15 分钟预算永不生效）；② `streaming-executor.awaitRemaining()` 有 30 分钟 backstop（工具不返回时回合仍往前，但工具本体成孤儿副作用）；③ 就这些。**v43 后的症状变化**：停止时记账注销 ⇒ UI 能回空闲、能发新消息（逃生舱），但那条 loop 永留栈上（`_loopDepth > 0`）——症状从「卡住」变成「幽灵轮」（新轮与它并发 + 并发留痕），根因未拆。落点需动重试循环核心（被遗弃的尝试仍可能往 executor 塞工具），要定参数并处理遗弃语义——单独立项，别顺手做 |
