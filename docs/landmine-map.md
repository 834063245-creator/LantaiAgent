...
🔸 **第三步已立项设计（2026-09-01，未施工）**：边界 schema——`typedJsonRpc` 的 `JSON.parse(raw) as T` 盲转无运行时形状校验（UI 全面审计批唯一挂起项，taste-ledger 2026-09-01）。设计件 [`../design/rpc-runtime-validation-design.md`](../design/rpc-runtime-validation-design.md)：zod 同址表 + 重载并存渐进收编，未登记命令直通；宪法病根一根治的收尾半步。
