...
| V3 | **野视口会被持久化**：`scheduleCanvasSave` 把 view 原样写进 `{workspace}/.lantai/canvas.json`，读回时 `restoreView` 直接用 | 任何一次跑飞（V2 的失控滚、手动滚远、拖块远钉）都把「卷外」的位置存进盘：下次进工作区**直接落在空白**（实测该文件里出现过 `panY=84010`、`224011`，而内容在 `-28630` 以远）；恢复优先于落位（有 `restoredView` 就不落锚）⇒「偶发」表现为**反复空白** | 用户「偶尔不加载」的第二个来源。**未做**：没有「恢复的视角若不含任何卷则视为无效、回落到回锚」这类校验 | ⚠️ 未修（候选修法：读回时判「视口内无任何流区」→ 回到活跃卷锚；或存盘时夹到内容范围。**要不要做属产品取舍**——留档待裁定，见 docs/plans/README.md 待办行） |
| V4 | `edge-scroll` 的悬停档**没有尽头**：滚到该方向再无内容也不停（`autoPanVector` 只按带深给速度，不认内容边界） | 指针停在屏顶/屏底带内 = 视口以 ~1,444 px/s 一路滚到卷外 ⇒ 屏幕空白（V2 的「正常形态」版：即使 mouseup 没丢，光是把鼠标搁在边上也会这样） | 2026-09-17 实测（探针 28）。**未做**：内容边界夹紧（RTS 相机滚到地图边即停）——**要不要加属手感取舍**，留档待裁定 | ⚠️ 未修（候选：帧循环里按「内容范围 ∪ 一屏呼吸」夹紧，保住「拖块远钉」这个被设计过的能力） |
