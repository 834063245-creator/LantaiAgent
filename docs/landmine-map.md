...
| L5 | `agent/agent-session-state.ts` 运行态账本 × `agent/tools/subagent.ts:142`（async 派发）× `shell` 域后台任务 | 「本卷在跑」的判据 = 运行账上的活记录（v43 起唯一事实），而 async 子 Agent 的生命周期**刻意独立于父轮**（`subagent-spawn.ts:119` 注释：async 模式合并中止源时排除父轮 signal），后台 shell 任务同理 | 父轮已收尾、活体子 Agent / 后台任务仍在推进：卷里 SubAgentPart 卡在「执行中」、任务输出还在长，用户眼里这个会话在跑；而创作坞无停钮、呼吸线不亮、书眉无「行卷中」，退出守卫（`shell/rows/persistence.ts` 的 `countRunningSessions`——同一把尺子）也不拦 ⇒ 关窗即丢在途工作 | ⏳ **产品语义待拍板**（机制已就位，v43 后是一行策略）：运行记录带 `kind`（turn/wake/goal/compact/subagent），「算不算在跑」从此是**读面策略**而非新账——候选 ① async 派生时给父卷账本寄一条 `kind:'subagent'` 记录（池活体计数归零时注销）② `bg` 后台任务寄 `kind:'bg'`。两者都只需在 `subagent-spawn` / `shell` 派发点加一次 begin/end，读面按 kind 决定停钮/呼吸线/退出守卫是否计入。<br>**2026-09-22 半边收口（役册）**：用户报「我没有一个能监控后台任务的地方」→ 落 **创作坞·役册**（坞内台账，设计规格书 §9.4）。补的是**看得见**（按会话的在役/已了清单 + 逐条停 shell + 他卷段），**不动本题**——「算不算在跑」仍是本条的待拍板项，呼吸线 / 停钮 / 侧栏状态点 / 退出守卫照旧只读运行账。 |
