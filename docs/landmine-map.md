...
✅ **第一步已落地（2026-08-22，commit be8bba85）**：① Rust 出口 Value 化——rpc 命令返回 `Result<Value, String>`，commands 层零改动，出口 `str_to_value` **纯包装 Value::String、故意不 parse**（出口无法区分「JSON 命令输出」和「恰好长得像 JSON 的文本」——read_file_content 读 .json 文件必须字节精确，回归测试钉死）；② 前端 `typedJsonRpc<T>`——typedRpc+parseJson 的组合收敛单函数，20 处调用点双重编码消灭；agentInvoke 直通 string，agent 工具链零改动。真机验证：CDP 实测 invoke 返回 string 形态字节精确；shell_env 返回 bundled:true（init_bundled 主路径首次应用内实证）。
🔶 **第二步（待排期）**：JSON 命令升级为真结构化 Value（ok_json 产 Value、文本命令保持 Value::String）——需在 Rust 侧建立「命令→形态」可靠分派（静态分析不可靠：commands 层直接 `Ok(json.to_string())`，静态识别不了），候选路径 = commands 层逐域改造或在 ok_json/ok_unit 辅助函数处收敛包装点。前端届时只需改 typedJsonRpc 一处（去 parse 透传真值）。
