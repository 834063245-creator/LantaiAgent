...
| L5 | `agent/agent-session-state.ts` 运行态账本 × `agent/tools/subagent.ts:142`（async 派发）× `shell` 域后台任务 | 「本卷在跑」的判据只有**一本主 Agent 的 exec 账本**——而 async 子 Agent 的生命周期**刻意独立于父轮**（`subagent-spawn.ts:119` 注释：async 模式合并中止源时排除父轮 signal），后台 shell 任务同理 | 父轮已收尾、活体子 Agent / 后台任务仍在推进：卷里 SubAgentPart 卡在「执行中」、任务输出还在长，用户眼里这个会话在跑；而创作坞无停钮、呼吸线不亮、书眉无「行卷中」，退出守卫（`shell/rows/persistence.ts` 的 `countRunningSessions`——同一把尺子）也不拦 ⇒ 关窗即丢在途工作 | ⏳ 未拆（**2026-09-20 运行态批未动**：属产品语义决策——「父轮已收尾但仍有活体后台工作」算不算在跑，需用户拍板；机制侧两条候选：① async 派生给本卷账本记一次「挂账」（需 ExecState 加不计入 controller 的计数位）② 子 Agent 池活体计数经 `_uiSessionId` 汇总成独立读面） |
