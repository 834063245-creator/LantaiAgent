...
> **本件是 ⏸ 搁置项**（真面板并排 / 多 core）——**重启该批时从归档取回并重新立项**（搁置裁定与四道拦路石见本文 §0.5/§0.6）。

> **状态：⏸ 搁置（2026-09-16）——已裁定但暂不启动。复核确认：preset 对本批零依赖，且本批买不到组合隔离（那部分 S6 已交付）。本件只立契约，未动一行代码；搁置理由与复活条件见 §0.6。**
...
### 0.6 搁置裁定与复活条件（2026-09-16）

用户提问「preset 一定要依赖这个多实例共存么」——**复核结论：零依赖，且依赖方向相反**。
证据（本地实测 **3 文件 18 例全绿**，2026-09-16）：

| 已成立（= 用户原画要的东西） | 证据 |
|---|---|
| 两卷各持一份组合；拨 A 只重建 A，B 的句柄引用与记录一动不动 | `tests/paper-side-by-side-agents.test.tsx` ① + ② |
| 两枚卷首芯片并排、各自显示各自组合名与来源（读面走真 ChatCore） | 同上 ③ |
| 同一 fs 工具实例按卷落不同 provider（A=本地 / B=替换） | `tests/seam-composition.test.ts` ⑨ |
| subagents / loopEvents / llm 方言各按本卷组合 | 同上 ⑩ / ⑪ / ⑬ |
| 会话数不进任何单卷可见面（并存只进激活账） | `tests/composition-session-count-profile.test.ts` 2 例 |

而本批（S7）买到的是**独立视口/缩放、独立活跃卷、面板级布局持久化**三项 UI 便利，
**买不到组合隔离**（设计件 §8.5 事实 3 原话；本单 §2.3 已记）⇒ **preset 不依赖它，
它也不给 preset 添任何能力。**

**⇒ 裁定：暂不启动。** 理由是**机会成本**而非难度：

1. **零前置**——没有任何东西等它（与 P0.5 之于 P1 不同：那条是真挡路）；
2. **用户原画「左审查右施工、各自显示自己的组合名」今天已交付**（上表）——S7 不是
   需求冒出来的，是交接里自生的一个选项（把「并排」又往「独立视口」解释了一层）；
3. **真实增量窄**：「同时以不同缩放看两个区域」；「独立活跃卷」的实际收益接近零
   （一次也只能往一个框里打字）；
4. **成本是全清单最高的一项**（真成本 = 参数化无 storeId 的应用级单例 `useCanvasViewStore`，
   7 文件 ~40 处 + 冻结基线 + 面板容器重做 + 契约升版）；
5. **用户 2026-09-16 处于决策过载状态** ⇒ 此刻开一个"听起来好"的最贵项，是应当拦住的。

**复活条件（三者任一，届时直接开工、不必重新论证）**：
① 用户实际使用中反复出现「想一边放大细看某卷、一边概览另一卷」的诉求；
② 出现真正的多工作区/多窗口诉求（届时 B3/B4 改法须一并重画）；
③ `useCanvasViewStore` 因别的原因被参数化（成本前置已被别人付掉）。

**更便宜的替代（独立于本批，值得先试）**：**按卷记住视口 + 聚焦时恢复**——今天的聚焦链
（`pendingFocusId` / `viewFocusRegion`，`canvas-view-store.ts:32` 与 `use-paper-focus.ts`）
已经在了，**无需多实例、无需动 ~40 处消费点**，用一次小改动拿到「切到审查卷就自动回到它
合适的倍率」这个 80% 价值。**此项由 Agent 自裁验证，不占用户带宽。**
