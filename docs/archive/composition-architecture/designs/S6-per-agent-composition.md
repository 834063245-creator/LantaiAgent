...
- **2026-09-16 用户真机验收 S6 P5 卷首组合芯片：通过**（原话「卷首那颗组合芯片我试过了，目前看来没问题」）。
  口径记录（如实，不过度外推）：验收对象 = 出厂产物（`paper-shell` 的 `FolioCompositionChip` + 产物 CSS），
  **须 `cargo tauri build` 后可见**（dev 态看不到产物插件改动）。用户明确确认的是**卷首芯片本身**——
  它正是本批用户可见的主件，承载三态语义（空白卷可拨 / 跑过一轮只读 / hover 来源）与三条落位纪律
  （绝对定位覆盖不进高度流水 / 只放开 chip 子树 / 读判写全经 core 能力位）。
  **未单独口头确认的两项（不在话里就不替用户签）**：① 两卷并排各自显示各自组合名
  ② 远档（`lodFar`）看不到芯片。前者有进程内证据兜底
  （`tests/paper-side-by-side-agents.test.tsx` ③「两枚卷首芯片并排：各自显示自己的组合名与来源」，
  读面走真 ChatCore）；后者是既有钉值（`tests/paper-lod-tiers.test.tsx` 远档卷首数 = 0，本批零改动）。
  **⇒ 本件至此无挂起的真机门**（P1e 创作坞芯片 + P5 卷首芯片两门皆闭）。
