...
> **已归档（2026-09-16 · 文档面重构）**——全计划竣工（2026-08-31：P0 + P1 + 增补一二三四全部落地，kind='feature' 全量通道化 + 位移式装载机制；dev 不可用实锤后生产热更升格唯一路径）。现状指针：`PLUGINS.md` + `docs/plugins/README.md`；现状入口 `docs/plans/README.md`。

> 状态：✅ **全计划竣工（2026-08-31：P0+P1+增补一二三四全部落地）**——面向兰台（Lantai）
