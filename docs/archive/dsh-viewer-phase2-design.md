...
> **已归档（2026-09-16）**：3D 视图随主仓图谱渲染内核退役从 dsh-bundle 拆除，本文是当时的设计记录，**不是现状**。
> 现状 = dsh-bundle 只提供「引擎 + MCP 工具面」；拆除裁定与 B3 收口见 `docs/landmine-map.md`。
>
> 状态（当时）：两块已摸透，待用户定「挂法」。本文件是唯一事实来源。
