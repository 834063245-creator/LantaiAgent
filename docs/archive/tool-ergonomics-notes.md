...
> **已归档（2026-09-16 · 文档面重构）**——T-1 + T-2 已落地（2026-08-30 当日实施竣工；rev2 归位 JS 平台层、Rust 零改动）。现状指针：`CONVENTIONS.md`（工具层纪律）+ 代码真源 `src-ui/src/agent/session-context.ts`；两份设计件见 `docs/archive/tool-ergonomics/`。
