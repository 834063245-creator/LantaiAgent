...
> 行表 12→9，双 preset 零漂移实测）；**S4-4 甲已毕（2026-08-23）**——插件贡献行/段进
...
> 勘定依据（四条机制约束，改代码前先对照）：
> ① **组合解析域边界**——~~roster/preset 只寻址 builtin 行~~ **已解除（S4-4 甲落地，
> 2026-08-23）**：factoryComposition() 快照收编 pluginToolRows 行 + ctx.prompts 段贡献
> ——patch/preset 可寻址 plugin/<贡献 id> 行与贡献段 id（含 13 第一方段）；寻址粒度
> = 单工具行（原 builtin/<族> 整族行 id 退役不复活）。①b web/browser-desktop 迁移
> 的寻址前置就此就位（minimal preset 寻址改为枚举 plugin 行 id，迁移批执行）。
...
| ①b | web | **已毕**（2026-08-23，与 browser-desktop 同批收官）：web（familyContributions 无状态族单工具行 `plugin/hologram/web-domain/web_fetch`）+ browser-desktop（**整组缓存行** `plugin/hologram/browser-desktop-domain/tools` 一行承载 53 工具——动态 import 族，不收 rowCtx 实例缓存跨装配复用；per-tool 名清单会锁死名面，整族行寻址恰是 minimal 禁整族的原语义）迁 ctx.tools。**builtin 行表全量迁毕退役**（`builtinToolRows()` 终结，tools 域唯一行源 = pluginToolRows()；无通道环境 = 空行表，寻址 plugin 行的解析须在 withFirstPartyToolChannel 腰内——生产时序 = loadBuiltinPlugins 先于 bootShell 组合链）。minimal preset 寻址改枚举 plugin 行；convergence 双 preset 零漂移 + tool-contract 文档零变化（装配序保住：web/browser-desktop 前插 firstPartyToolPlugins 清单首） |
...
| ② | 工具大域 | 分族：**fs / shell / agent-isolation 已毕**（P4 ②，2026-08-23：同 ①a——无状态 codingExec，通道现成；plugins/coding-domain-plugins.ts 五族合并文件 + first-party-tools.ts 清单扩展，行表 12→9，convergence 双 preset 零漂移实测；行 id builtin/fs・shell・agent-isolation 退役——S4-4 甲已落地：贡献行 plugin/hologram/<域>-domain/<工具名> 全量可寻址）；**browser-desktop 已毕**（随 ①b 批 2026-08-23 收官：整组缓存行 `plugin/hologram/browser-desktop-domain/tools`，minimal 寻址 plugin 行）；memory / skill / task / agent **已毕**（随 ①c 批 2026-08-23） |
...
| ⑤ | 会话级能力（plan/通信/discovery/merge/board/compaction） | **已毕——收官（2026-08-24，B⑤）**：十五项第一方 capability 全量经 ctx.capabilities 通道贡献（A-3 通道的消费面兑现）。`plugins/capability-segments-plugin.ts` 装载 `agent/blueprint.ts` 的 `firstPartyCapabilities()`（序 = 迁移前出厂表序，单批零漂移按构造——converge-tools 与 code-execution-tool 双双 install 期快照 visibleTools() 的表序依赖由清单序保住，中间分批会击穿该依赖故单批收官）；`composition/first-party-capabilities.ts` 是清单单一真源 + 无引导环境装配腰（withFirstPartyCapabilityChannel——convergence 夹具/gen-tool-contract 复现生产 capability 面）。出厂 `builtinCapabilities()` 退役（B④ builtinPromptSections 同款终态——capability 定义仍留 blueprint.ts，改名 firstPartyCapabilities() 供插件装载）；`AgentBlueprint.standard()` 快捷方式随之退役（零生产调用方，标准面 = 通道快照经 fromRoster 派生——S2-1 既有穿线零 runtime 改动）。配套：blueprint.test 表序断言换代钉通道面、composition-wiring 不变式改钉新真源（通道快照 ≡ 清单序）、gen:tool-contract 叠 capability 通道腰（文档字节零变化实测）。撞 builtin key 拒绝随出厂表退役（注册表重名拒绝 + 装载序即防线——B④ prompt-service 同款终态） |
| ⑥ | 管道参与（graph hooks/board tracking/preflight） | **已毕（收官 2026-08-24，B⑤）**：外部插件钩子通道 A-2 ✅（2026-08-24）；第一方钩子件 graph-hooks/board-tracking-hook 随 B⑤ 迁 ctx.capabilities 通道（不再是 builtin capability 表项）——B⑥ 行剩余语义（第一方件随 B⑤ 先行批迁移）已兑现，行毕 |
