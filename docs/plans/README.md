...
| 画布空间模型 | [`canvas-space/canvas-space-model-notes.md`](canvas-space/canvas-space-model-notes.md) | **设计定稿（2026-08-25），施工顺序见笔记 §7，待开工** | 一整片会生长的纸 / 有界流区（统一宽度·线性排比）/ 宿主模型（依附物跟宿主走）/ 三层导航（工作区→侧边栏+画布→书脊+创作坞）/ 性能基准（上百会话+N Agent 同跑）/ 施工六阶段+五原则；阶段性展开文档将落 `canvas-space/` 子目录 |
