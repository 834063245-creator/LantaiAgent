...
| **会话流版式节奏（stream-rhythm）真机验收五项** | 代码四刀全落地（2026-09-03，门禁全绿），待实机：① 长回合（≥10 次工具调用）读起来是几个工作单元 + 换气，不是等距瀑布；② Error 出现处明显转折（前置放空），Retry 紧贴其后；③ 流式中活尾重排不引起视口上方跳动，回合 finalised 后永不再动；④ 折叠组展开后文件路径判别量可见（折叠行露判别字段）；⑤ 旧卷回放（任一历史长卷）过新管线渲染正常、滚动性能不退——另带 **D1 间距三档（32/64/96）与 D2 阶段细线形态终审**（taste-ledger 待用户判）——见 [`stream-rhythm-plan.md`](stream-rhythm-plan.md) §5 |
