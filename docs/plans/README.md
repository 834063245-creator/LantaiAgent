...
| 引擎插件化 | [`engine-plugin-extraction.md`](engine-plugin-extraction.md) | **In progress**——**Phase 0（契约 v2）+ Phase 1（引擎侧 10 个壳方法）+ Phase 1.5（分页栈整链拆除，graphData→快照）已落地**（`d2fa1a6e` / `c08686cc`，净删 912 行，门禁全绿：引擎 584/壳 417/vitest 1883/convergence 零漂移）；**Phase 2（transport 抽象）施工约 90% 未 commit**——接手序列与环境坑（DSH 常驻引擎进程勿杀/管道假挂）见计划 §Phase 2 交接块；bin 测试 27 个拆除（TCP 旧协议面 Phase 3 本就拆） | 把图谱引擎拆成独立插件（2026-08-23 已定「Rust 侧插件化标准形态 = 外部 MCP server」兑现）；DSH 不共包 / monorepo 子目录已拍板；全仓旧时代残留清点见计划 §8 |
