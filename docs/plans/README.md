...
| **多模态图片链路（附图入卷）** | 3-5 天 | **In progress：B1 底座 + B2 采集面已竣工**（2026-09-08，commits 0d2c39bd/b99c5217，门禁全绿——ChatImageRef 引用模型（字节永不进卷）+ fs_cap write_base64 能力口 + image-intake 准入规整 + 粘贴/拖放/夹选三入口 + 附图 rail 缩略/lightbox + vision 目录门禁（策略在创作坞、机制在 chat-core）；**B3 发送**（agent.run 扩参 + 请求期解析 + 三适配器 content parts + 预算降级 + 能力投影）→ **B4 渲染**（用户气泡缩略 + markdown img + measure 镜像）→ **B5 配置**（catalog vision 声明 + ModelOverrides.input + ProviderDetail 开关）待施工——见 [`multimodal-image-plan.md`](multimodal-image-plan.md) |
