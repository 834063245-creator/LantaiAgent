...
| 线 | 文档 | 状态 | 说明 |
|---|---|---|---|
| 平台化 | [`agent-platformization-plan.md`](agent-platformization-plan.md) | **Phase 1 + Phase 2 竣工**（2026-08-27，七 commit：`98a6f30d`/`605d6e8a`/施工③/`4c7b6467`/`4bcd0c05`/`daae88c3`/收口；D2/D5 修订 A 路线拍板 + D11 修订 subprocess 并入），Phase 3 组合域统一待开工 | 强制层/能力契约层二分宪法已入档；Phase 1 swappable seam（ctx.llm/ctx.subagents/13 事件表 + feature→mechanism map）；Phase 2 后端能力四 seam（fs/shell/sessionPersistence/graph，默认 provider = Rust/engine 恒等包装；subprocess 并入 shell）；P2-C3 强制层不旁路守卫钉死；拆旧清单 T1-T11 清零，门禁四连全绿 |
