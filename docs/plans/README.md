...
| 项 | 成本 | 说明 |
|---|---|---|
| **第一方代码热更（P0+P1 已批留档）** | P0 半天 · P1 2-3 天 | P0 开发热更工作流（dev.cmd + dev-workflow 文档，Vite HMR 即刻生效）；P1 渲染器插件通道化（媒体渲染器迁内置插件 + 宿主桥加 React + esbuild 产物 + Rust 兜底解析 + 设置面板重载入口；生产包也可热替换）。**已批准留档，等指令开工** —— 见 [`first-party-hot-reload-plan.md`](first-party-hot-reload-plan.md) |
