...
| 项 | 成本 | 说明 |
|---|---|---|
| **会话持久化 seam 接线（C 定案，plan 已落待下窗施工）** | 1-2 天 | R5 落账窗发现空承诺（`sessionExecute` 产线零调用方——唯一设计消费方 agent-store 2026-09-01 已内存化；真实落盘链全在 seam 外直连）→ 用户拍 **C 把承诺做实**：四动作面重设计（read/list/save/delete volume）+ 默认 provider 走 kernel* 具名 helper（测试 mock 面零迁移）+ chat-session/chat-core 消费接线（冻结文件最小 diff，施工⑥范围注记明文 deferred 的 P5 后续）+ 开放面契约升 v13——plan 见 [`session-persistence-seam-wiring-plan.md`](session-persistence-seam-wiring-plan.md) |
