...
| **会话树（枝）** | 中（app 层为主：节点判据 + 头行血缘与边归一化 + 树面与连坐 + 画布承接，三批 + 可选一批；不动事件词表） | **P1 / P1′ / P0 修 / P2 / P3 已落地（2026-09-18）**：消息动作行「立枝」→ 新枝卷摊到纸上**且视角飞过去**；侧栏树形 + 书脊「枝」标 + 未落定置灰；删父卷**连坐整棵子树**（后序、逐卷可见）；画布上一丝朱砂引线连回父卷的分叉节点、点线溯源。**剩 P4（可选：空间手势立枝 / 旧枝考古）未开工**。裁定见 §9、施工记录见 §12——[`session-tree-plan.md`](session-tree-plan.md) |
