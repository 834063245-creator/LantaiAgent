...
| 引擎插件化 | [`engine-plugin-extraction.md`](engine-plugin-extraction.md) | **Proposed·Draft（2026-08-29 立项）**——兰台进程外 + MCP 契约：引擎进程 = 唯一运行时实体（stdio MCP + TCP 9777 + 免编译扩展面），兰台从内嵌 crate 改每工作区 spawn 引擎进程；前端 RPC 契约零改动，手术全在两个 Rust 侧。Phase 0（契约化）/1（壳专属方法）不依赖拓扑决策可先开工；Phase 2/3（transport + 内嵌退役）待拓扑拍板（每工作区一进程 推荐） | 把图谱引擎拆成独立插件（2026-08-23 已定「Rust 侧插件化标准形态 = 外部 MCP server」兑现）；DSH 不共包 / monorepo 子目录已拍板 |
