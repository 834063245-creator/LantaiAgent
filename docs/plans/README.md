...
| 项 | 成本 | 说明 |
|---|---|---|
| **插件 bundle 退役（30 个出厂插件全产物化）** | 中等（3-5 天） | **Proposed·Draft（2026-09-03 凌晨用户拍板）**——「所有的都不进编译」：七个 seam 供应商 + 23 feature 全部产物化随安装包分发，编译进 exe 的插件形态整体退役（exe = 装配台：装载链 + 10 注册表 + 4 平台运行时）；23 feature 已产物化（增补四竣工），本计划收口剩余七供应商 + bundle 双轨废除；三决策点已定案（2026-09-03 DSH cordis 调研）：无懒装载、装载 = 依赖图 + boot 全 ACTIVE 审计（fail-loud）、三层源 = patch 覆盖语义，见 [`plugin-bundle-retirement-plan.md`](plugin-bundle-retirement-plan.md)（§4）——改插件 = 换产物，永不重编译 exe |
