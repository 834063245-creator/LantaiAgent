...
| **会话树（枝）** | 中（app 层为主：节点判据 + 头行血缘与边归一化 + 树面与连坐 + 画布承接，三批 + 可选一批；不动事件词表） | **P1 + P1′ 已落地（2026-09-18）**：消息动作行「立枝」可用（与「改 / 重发 / 抄」同行）——节点自包含（每枝一卷 = 父卷前缀复制）、边落头行、父卷字节零变化；**P2 未开工**（删除连坐 + 侧栏树形 + 未落定置灰）。裁定见 §9、施工记录见 §12——[`session-tree-plan.md`](session-tree-plan.md) |
