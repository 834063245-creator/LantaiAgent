...
| **内核插件运行时** | [`kernel-plugin-runtime-plan.md`](kernel-plugin-runtime-plan.md) | **In progress：Phase 0 契约+注册表+tool_call 统一入口 + Phase 1 builtin.search + Phase 1 续 builtin.web 已落地（2026-09-04：web_search/web_fetch 迁毕、commands/web.rs 退役、coding.ts 死码清理）→ Phase 2 工具域全量迁移 **已竣工**（2026-09-04 窗：P2-0 权限基建 08c5466f + P2-1 constraints/editor 87ab054a + P2-2 fs 主体 fcd120b4 + P2-3 git e2593712 + P2-4 shell+进度流 b74d18c6 + P2-5 browser/uia c12281e8 + P2-6 pty/lsp d419c9d7——九域全部信封化，rpc 45 methods；权限形状：P2-5/P2-6 按设计件 §8 裁决（browser/uia/pty/lsp 不进 manifest permission，插件内业务自检/高层函数直调））→ Phase 3 内核插件管理面（扩展现有插件 tab，非另起 UI：plugin_tool_manifests 消费 + 启停持久化 + 能力授予 + 第三方内核插件装载） | 用户 2026-09-03 拍板：Rust 内核不再实现任何具体工具，全部走 ToolPlugin（可装可卸、能力声明、内核统一管控权限/沙箱/审计）；前端只消费 Tool Manifest；RPC 分支按批退役收敛到 tool_call + 生命周期族（145 → 142 → 129） |
...
| **Agent 资产块（块协议落地）** | 已竣工（[`agent-asset-blocks.md`](agent-asset-blocks.md)）——WO-1..WO-8 全 ✅ | **设计定稿 + 拍板全齐 14 项；WO-1 ✅（协议与类型）WO-2 ✅（工具三件套 + executor 资产通道 + baseline 获批 record）WO-3 ✅（block-model 开放 + translate 映射）WO-4 ✅（兜底 JSON 渲染 + 文类签）WO-5 ✅（资产表 + update 广播 + pinned 孤儿刷新）WO-6 ✅（首发表现原语）WO-7 ✅（持久化/回放）WO-8 ✅（html 沙箱）**——Agent 随时生成可引用/更新的资产块：show_asset / update_asset / list_block_kinds 三原语 + BlockPart + **语义 kind / 表现 presentation 双维度正交**（7 族判据 + 13 表现原语 + 会话级资产表 A7 + html 沙箱逃生舱；graph 轻量 SVG 兑现图谱内化 ADR） |
