...
| **目次带加宽（缘滚跑道）手感** | 代码已落地（2026-09-19 加宽批 + 同日甲案，门禁全绿；病灶与几何见 [`../design/lantai-design-spec.md`](../design/lantai-design-spec.md) §13）——待用户体感确认四件：① 带体 104px（识别层满宽 + 控制列 64 + 缘滚跑道 40）在正文之上是否碍眼、是否压到你想看的内容；② **读带/拖带时视口是否不再自己跑**（控制列在缘滚感应带外）；③ 贴屏最右（跑道上）是否照旧起滚，且**不再弹 hover 卡、点击不再跳视口**（跑道惰性）；④ 甲案观感：识别层铺满 + 界栏线分界是否合意、界栏线浓淡是否合适（要更宽/更窄 = `TOC_RUNWAY_W` + CSS `.pp-toc` 的 width + 浮件右距三处同改，钉值会红）。**须重建 exe**（这一版插件 CSS 得进壳 bundle；H2/H3 修好后，之后的 CSS 微调走三步热更——边界见 [`../dev-workflow.md`](../dev-workflow.md)）——owner：用户 |
