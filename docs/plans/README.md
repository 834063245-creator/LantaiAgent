...
| 引擎插件化 | [`engine-plugin-extraction.md`](engine-plugin-extraction.md) | **In progress**——**Phase 0（契约 v2）+ Phase 1（引擎侧 10 个壳方法）+ Phase 1.5（分页栈整链拆除，graphData→快照）+ Phase 2（壳侧 transport 抽象 + 差分对拍）已落地**（`d2fa1a6e` / `c08686cc` / Phase 2 本窗落账，门禁全绿：引擎 584/壳 bins+lib 420 + 集成 16/前端全量/convergence 零漂移；对拍抓到 2 个契约面真 bug 已修）；**下一步 Phase 3（翻默认 transport=McpRemote + 壳摘 hologram-engine Cargo 依赖 + legacy MCP RPC 面拆除 + McpManager 退役）**；环境坑（DSH 常驻引擎进程勿杀/PS 重定向假挂/cdp 僵尸 chrome）见计划 §Phase 2 + AGENTS 测试纪律；bin 测试 27 个拆除（TCP 旧协议面 Phase 3 本就拆） | 把图谱引擎拆成独立插件（2026-08-23 已定「Rust 侧插件化标准形态 = 外部 MCP server」兑现）；DSH 不共包 / monorepo 子目录已拍板；全仓旧时代残留清点见计划 §8 |
