...
| 线 | 文档 | 状态 | 说明 |
|---|---|---|---|
| 平台化 | [`agent-platformization-plan.md`](agent-platformization-plan.md) | **Phase 1-3 竣工**（2026-08-27，十 commit；Phase 3 三 commit：`1505272f` seam 裁剪域进解析域 / `68f1d2e5` 服务·事件目录生成器 + doc-sync / `21ff8324` 开放面契约版本化 + plugin_install 版本比较），Phase 4 运行时插件全链路待开工 | 强制层/能力契约层二分宪法已入档；Phase 1 swappable seam（ctx.llm/ctx.subagents/13 事件表）；Phase 2 后端能力四 seam（fs/shell/sessionPersistence/graph）；Phase 3 组合域统一（patch/preset 可寻址禁用/替换 seam provider + loopEvents 事件面开关；服务/事件目录生成 + doc-sync 门禁；开放面契约版本 v1 + 指纹对拍；拆旧 T-P3-1/2/3 清零） |
