...
| **软件级插件（app shell 四件套）** | 7-10 天 | 2026-09-03 立案，**2026-09-05 决策点全定稿待开工**：窗口原语（iframe 真隔离 + postMessage 白名单桥）+ 插件数据目录 + 受治进程治理（决策 8 收敛：受治进程 = MCP server 一种，mcp-bridge 升级；工具只有工具口 / MCP 路两张门）+ 后台唤醒回调；范本示例 `examples/plugins/notes-app/`——见 [`app-shell-software-plugin-plan.md`](app-shell-software-plugin-plan.md) |
