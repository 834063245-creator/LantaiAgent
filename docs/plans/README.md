...
| 项 | 成本 | 说明 |
|---|---|---|
| **工具层人体工学 + 工具层智能** | 两批 ~2 天 | notes [`tool-ergonomics-notes.md`](tool-ergonomics-notes.md)；**设计件 ×2 已出 rev2（2026-08-30）**：[`tool-ergonomics/design-1-context-waist.md`](tool-ergonomics/design-1-context-waist.md)（JS 上下文腰：per-owner 注册表 + 相对路径/省缺填充/可见键归一 + 描述瘦身）+ [`tool-ergonomics/design-2-session-focus.md`](tool-ergonomics/design-2-session-focus.md)（fs 焦点文件 + desktop 粘性窗口，sticky_cwd 语义四要素 JS 化；graph nodeId 裁决不做）——**Rust 零改动**（用户质询插件化纪律后 rev2 重裁：enforcement 留 Rust，resolution 归 JS 平台层），自查模式待实施，同窗连续两批共享 baseline freeze |
