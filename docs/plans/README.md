...
| 引擎插件化 | [`engine-plugin-extraction.md`](engine-plugin-extraction.md) | **In progress——Phase 0/1/1.5/2 已落地；Phase 3 第一步已落地（默认 transport=McpRemote + legacy MCP 面拆除：`mcp_manager.rs`/start·stop RPC/McpService 全退役，frontend-rpc-contract 156→151）**（`d2fa1a6e`/`c08686cc`/Phase 2 本窗 `e662648f`/Phase 3 第一步 `9a920490`；门禁全绿：引擎 584/壳 bins+lib 417+集成 16/前端 203 文件 1895/convergence 零漂移；顺手修 D13 loopHost pendingInserts 别名死循环生产 bug + registry 测试 env 竞态）；**下一步 Phase 3 第二步 = 摘 hologram-engine Cargo 依赖（施工清单见计划 §Phase 3 十项：notification 泵/run_check 壳方法/is_ignored_path 归置/DataContext 去 Engine 等）——建议真机验收 McpRemote 默认路径（env=inprocess 逃生口仍在）后再动刀** | 把图谱引擎拆成独立插件（2026-08-23 已定「Rust 侧插件化标准形态 = 外部 MCP server」兑现）；DSH 不共包 / monorepo 子目录已拍板；全仓旧时代残留清点见计划 §8 |
