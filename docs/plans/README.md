...
| 项 | 成本 | 说明 |
|---|---|---|
| **插件 bundle 退役（30 个出厂插件全产物化）** | ✅ 已竣工（2026-09-03，S1-S6 六 commit） | **S2-S5 全部落地**：六供应商真源产物化 + 十六工具域/两段贡献薄壳→真源 + 装载调度层（依赖图 PENDING + boot 全 ACTIVE 审计 fail-loud）+ bundle 双轨拆除（BUILTIN_PLUGINS 44→15 内核，29 出厂产物走磁盘通道，displace 退役，dev/prod 双态 import.meta.env.DEV 隔离）。**唯一偏差**：agent-loop-service 暂缓产物化（模块级状态共生，内核 14+1=15）。**真机验收五项待用户跑**——见 [`plugin-bundle-retirement-plan.md`](plugin-bundle-retirement-plan.md)（§4） |
...
| **会话流版式语法（stream-rhythm）** | 4-6 天 | **In progress：刀1+刀2 已落地（2026-09-03，门禁全绿，真机验收待用户跑；刀3 封口纪律+长会话 / 刀4 小修收口下一窗）**——事件语义分类（`paper/grammar.ts`：工具调用→读/写/验证/提交派生族，未知名兜底不破语法）+ 跨块工作单元（`paper/group.ts` 封套 pass）+ **版式语法表数据化**（补 seal/host 两动词）+ 节奏渲染（单元内紧 32 / 单元间 64 / Error 与阶段放空 96 + 阶段细线 --rule-soft）+ 两宪法（封口纪律：已封口单元不可变；跨消息前瞻禁止：单元不跨消息，消息边界即收口）——见 [`stream-rhythm-plan.md`](stream-rhythm-plan.md)（原方案全文存其附录 A） |
