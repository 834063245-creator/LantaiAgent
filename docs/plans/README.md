...
| **内核插件运行时** | [`kernel-plugin-runtime-plan.md`](kernel-plugin-runtime-plan.md) | **In progress：Phase 0 契约+注册表+tool_call 统一入口 + Phase 1 builtin.search + Phase 1 续 builtin.web 已落地（2026-09-04：web_search/web_fetch 迁毕、commands/web.rs 退役、coding.ts 死码清理）**→ Phase 2 工具域全量迁移（P2-0 基建 → constraints/editor → fs → git → shell+进度流 → browser/uia → pty/lsp；原 Phase 2/3 已合并，设计件 kernel-plugin-runtime-phase2-design.md）→ Phase 3 内核插件管理面（扩展现有插件 tab，非另起 UI：plugin_tool_manifests 消费 + 启停持久化 + 能力授予 + 第三方内核插件装载） | 用户 2026-09-03 拍板：Rust 内核不再实现任何具体工具，全部走 ToolPlugin（可装可卸、能力声明、内核统一管控权限/沙箱/审计）；前端只消费 Tool Manifest；RPC 145 分支按批退役收敛到 tool_call + 生命周期族 |
