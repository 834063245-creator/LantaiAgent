...
| 引擎插件化 | [`engine-plugin-extraction.md`](engine-plugin-extraction.md) | **In progress**——**Phase 0（契约 v2）+ Phase 1（引擎侧 10 个壳方法）已落地**（2026-08-29，lib 584 全绿）：hidden tools 进 dispatch、watcher 事件桥下沉 `engine::watcher`（回调常驻不重启）、进度/watcher 推送走 MCP notification；bin 测试 27 个拆除（TCP 旧协议面 Phase 3 本就拆）；下一步 = **Phase 1.5 前端分页拆除 + graphData→snapshot**；**拓扑已拍板：A（每工作区一进程）**，Phase 2/3 按 A 落 | 把图谱引擎拆成独立插件（2026-08-23 已定「Rust 侧插件化标准形态 = 外部 MCP server」兑现）；DSH 不共包 / monorepo 子目录已拍板；全仓旧时代残留清点见计划 §8 |
