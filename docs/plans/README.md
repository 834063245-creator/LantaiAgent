...
| 引擎插件化 | [`engine-plugin-extraction.md`](engine-plugin-extraction.md) | **Phase 0/1/1.5/2/3 全部竣工（2026-08-29）——Phase 3 两步走收官：摘 hologram-engine Cargo 依赖（`src-tauri/Cargo.toml` 零引擎依赖，grep 清零），兰台全链路进程外消费引擎（每工作区一个 `engine serve` 子进程，stdio MCP）**（notification 泵 = EngineSession 常驻读线程 + 封顶队列，watcher 桥→graph-updated / progress→analyze-\*；契约 v3 = 11 壳方法：新增 `run_check` 上收 + `analyze_with_progress` 缓存新鲜度门；`is_ignored_path` 迁 hologram-graph；store_host 壳自开；进程级 e2e 双工作区隔离 + 崩溃重启持久化闭环；内嵌臂/差分对拍/回落臂/regenerate_file_graph 死链全拆，净 -800 行）。门禁全绿：图类型层 53+1 / 引擎 583 / 壳 411+集成 1 / 前端 203 文件 1895（provider-realsocket 满载时序 flake 复跑绿）/ biome 0/0 / convergence 零漂移；行为变更四条见计划 §Phase 3 竣工记录。**待真机验收：开卷/切卷/图查询/工具调用/merge gate/图 hooks 全链路 + 崩溃恢复不挂主进程；下一步 = Phase 4 免编译扩展面** | 把图谱引擎拆成独立插件（2026-08-23 已定「Rust 侧插件化标准形态 = 外部 MCP server」兑现）；DSH 不共包 / monorepo 子目录已拍板；全仓旧时代残留清点见计划 §8 |
