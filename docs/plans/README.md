...
| 日期 | 里程碑 | 工程 |
|---|---|---|
| 2026-08-22（深夜） | **V5 提前拍板：摘除旧观测台前端**——用户原话「这个狗日的旧前端就不能摘了吗，就非得挂在这」（taste-ledger 同日落账）。**显式翻案**：推翻「纸能住人 + R3 访谈后再 V5」的共居期排序；**R3 退役访谈作废**（拆除不等判据——退役判据无对象）。纸壳（注疏案卷工作台）成为唯一主界面。当日背景：兰台注疏定稿全量落地（7 commit 门禁全绿）+「纸面板即聊天」成立（composer 接 core.sendMessage，消息流经 translate.ts 转译为块）——拆除的可用性前提与审美前提同日齐备。拆除序列：① 落账（本条+taste-ledger）② graph preflight 摸影响面 ③ paper 落点唯一路径（boot.ts L95-101 preset 分叉 / presets.ts paper 行）④ 冷启动直落纸视图（cold-start.ts）⑤ 拆 CommandBar/DockRail/StatusBar、graph 视图 chrome、旧 ChatBeacon 聊天 UI、prompt-shelf 等旧 app 件 + shell-rows 对应行 ⑥「关纸面板回哪」按原型 home 全量转录重设计（现状 SessionsHome 只是半档重排，欠账一并清）⑦ 守护测试同步（ui-react-retirement / eventbus-zero-and-ui-split 等）+ 全套门禁 | paper-shell / 总控 |
...
**paper-shell V5：摘除旧观测台前端**（2026-08-22 深夜用户拍板，优先级高于 R5 打磨环；详细序列见上表当日条目）——纸壳成为唯一主界面。拆除完成后接 **R5 打磨环**：注疏定稿已全量落地（风格源 `prototype/lantai.html`，契约 `docs/design/lantai-design-spec.md`）；vision 会话跑自检循环（agent 对着黄金样本自我迭代，管线在 `D:\tmp\r5-vision\`——shot.mjs headless 截图 + read_image；本模型有图片输入，旧「agent 无图片输入只能靠用户喂眼判」结论已过时），用户只做终审；C 段产品化涌出项（旧 chrome 去圆角 / SettingsPanel 重排 / --obs-* 别名层退役等）随做。队列与规则见 [`paper-shell/r5-polish-backlog.md`](paper-shell/r5-polish-backlog.md)（B 段＋C 段），全部决定见 [`paper-shell/taste-ledger.md`](paper-shell/taste-ledger.md)。
...
| [`paper-shell/`](paper-shell/) | In progress（2026-08-20 立项，独立创作工程。V0 ✅ → 走查弹 ✅ → V3a ✅ → V3b ✅ → V5 机制半 ✅；V1 三轮负结果后经参照物板收敛：**R5 印刷品方向通过（2026-08-22，首个正判定）**，材质暂定 R4 零纹理；同日 **兰台注疏设计定稿 handoff 全量落地**（tokens/PaperPanel/首页/术语/favicon）；**深夜 V5 提前拍板——摘除旧观测台前端，纸壳成为唯一主界面**（R3 退役访谈作废）） | **V5 拆除**（序列见总控表 2026-08-22 深夜条目）→ R5 打磨环 B 段＋C 段（队列见 r5-polish-backlog.md） |
