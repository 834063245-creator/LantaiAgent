...
| 线 | 文档 | 状态 | 说明 |
|---|---|---|---|
| 平台化 | [`agent-platformization-plan.md`](agent-platformization-plan.md) | **Phase 1-4 竣工**（2026-08-27/28，十三 commit；Phase 4 三 commit：`5b676af2` D6 运行时热重载 / `7151086e` D7 ctx.dynamicRunner + cordis 域 + 契约 v2 / `40f119fb` D1 MCP 端到端例子 + D12 信任模型），Phase 5 存量迁移与出厂面清零待开工 | 强制层/能力契约层二分宪法已入档；Phase 1 swappable seam；Phase 2 后端能力四 seam；Phase 3 组合域统一（seam 裁剪域 + 目录生成 + doc-sync + 契约版本化）；Phase 4 运行时插件全链路（D6 装卸运行时生效；D7 动态插件 define/run/stop/undefine/inspect——approval + 沙箱三层防线；cordis 域入模型可见面；外部 MCP 端到端例子） |
