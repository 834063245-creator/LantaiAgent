...
| 引擎插件化 | [`engine-plugin-extraction.md`](engine-plugin-extraction.md) | **In progress**——Phase 0（契约化 v1）已落地（`520d8ae9`）；**2026-08-29 复盘砍分页**（为已退役 3D 星图 + IPC 护栏服务的双重死代码，整链删除）：契约 v2（删 3 壳方法加 `graph_snapshot`/`file_nodes`）+ 新增 Phase 1.5 前端分页拆除 + graphData→snapshot；下一步 = 契约 v2 + Phase 1 壳方法实现；**拓扑已拍板：A（每工作区一进程）**，Phase 2/3 按 A 落 | 把图谱引擎拆成独立插件（2026-08-23 已定「Rust 侧插件化标准形态 = 外部 MCP server」兑现）；DSH 不共包 / monorepo 子目录已拍板；全仓旧时代残留清点见计划 §8 |
