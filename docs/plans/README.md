...
| **多模态图片链路（附图入卷）** | 3-5 天 | **In progress：B1 底座 + B2 采集面 + B3 发送面已竣工**（2026-09-08，commits 0d2c39bd/b99c5217/9affbbf2，门禁全绿——引用模型（字节永不进卷）+ write_base64 能力口 + 准入规整 + 三入口采集 + 附图 rail + vision 门禁 + **发送边界管线**（能力投影 inputModalities/预算降级保新弃旧/请求期解析缓存）+ 三适配器 wire 构造（openai parts/anthropic image blocks/responses input_image，纯文本形态字节不变）；**B4 渲染**（用户气泡缩略 + markdown img + measure 镜像）→ **B5 配置**（catalog vision 声明 + ModelOverrides.input + 设置开关——落地后门禁自然放开）待施工——见 [`multimodal-image-plan.md`](multimodal-image-plan.md) |
