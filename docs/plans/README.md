...
| **组合粒度（per-agent composition）** | [`composition-architecture/designs/S6-per-agent-composition.md`](composition-architecture/designs/S6-per-agent-composition.md) | **P-1 + P0.5 + P0 已落地（2026-09-14）；P1 起待续** | 把组合从「全局状态的一次函数求值」推到「每 Agent 一份值」。**已落地（2026-09-14）**：**P-1 authoring 环境**（`composition_dir` RPC + 打开目录 + 复制内置为模板（拒覆盖 / id 围栏 / 内置 id 拒绝）+ 免重启重扫 `rescanPresets` + 设置面板作者块——单二进制下用户不动源码即可配 preset，四条缺项全补）；**P0.5**（minimal 轨重录 `office` 漏项 + `verify:convergence` 改双轨，CI 经同一 npm script 自动获得第二轨）；**P0 记录闭环**（卷落 `presetId` + 会话工厂按卷内记录的组合重建 + 恢复期校验与「本卷组合不可用」提示 + 连带修复改名不再抹掉 `tokens`/`compose`）。**待续**：P1 卷级选择（全局默认 + 卷级两层）+ 选择集（`defaultOff` 行 / `disabled:false` 回开）→ P2 装配期 provider 值注入（seam 选择去模块态）→ P3 插件按需激活（引用计数 + 独占声明 + fail loud）→ P4 程序入口（RPC/MCP 带 `preset`）→ P5 卷头 chip + 只读标签 + 同屏并排。路线 = **视图 + 引用计数**（不抄 DSH scope realm，realm 仅作资源型插件逃生门）；不变式 = **门禁维度只在出厂 preset（用户 preset 走运行时校验，空间无限）**。立项输入 = 2026-09-14 组合层审计（F1-F6 断链修复批已落地）+ 用户四项拍板（含「preset 不上线，平台只提供环境」） |
...
| 项 | 成本 | 说明 |
|---|---|---|
| **OfficeCLI 集成（office-cli）** | **工程完成，待用户真机验收**（2026-09-13 立项并十窗完工；goal 已 blocked/disarmed）：P0 平台前置（MCP 只读语义归真，契约 v27）+ P4 窗入口二态（契约 v28）+ A 路落位 + 载体插件 + 哈希校验安装器 + 活预览两形态（截图刷新 / 环回 URL 窗）**全部落地**；e2e 12 例真二进制绿、build 与 convergence exit 0。**余**：§6 真机验收（含唯一真机判据「Tauri 能否框环回页收 SSE」）+ 载体签核 + 产品方向件 | 把单二进制 Office 套件接进兰台（MCP 单工具 `officecli(command)`，argv 数组形态）。**实测**：win-x64 31.87 MB / v1.0.149（哈希已核）；`view screenshot` 可用、`view html` 外链 CDN、watch 页 SSE 同源——三条呈现路已核查（活预览要平台件，截图路立即可用）；发现并修掉平台洞「MCP 工具恒只读 ⇒ 写动作绕过 plan 模式与并发组」。见 [`office-cli-integration-plan.md`](office-cli-integration-plan.md) |
