...
> 状态：**P1 + P1′ + P0 修 + P2 + P3 + P4-③ 已落地（2026-09-18 / 09-19，施工记录见 §12）；
> P4-① 待开工；P4-② 不做（裁定与理由见 §9）**。
...
- **旧枝考古**（把今天已躺在日志里的旧枝读出来挂上树）：~~诱人，但只能得到**只读展品**~~
  ...
  （那条「历史还在」的发现已转为独立案子——判定件随本线收尾批单独提交）。本线不再提。
...
| **P4-① 空间手势立枝** | ⏳ **待开工**（用户 2026-09-19 裁定：做） | 单独立项时定 |
...
| **旧枝考古（P4-②）** | **不做**——用户 2026-09-19 裁定，判定「毫无意义」。理由（用户原话的意思）：**「改 / 重发」本身就是破坏性操作**，正常来说旧消息应当**直接抹掉**；今天「历史还在日志里」这件事本身就不正常，不是可挖掘的资产 | 撤销 §6 那条「留台阶」的诱因：日志里躺着的旧内容**不是**待发掘的枝，而是**待清理的残留**（后者单独立案：判定件随本线收尾批单独提交）；本线不再提「把旧枝读出来挂上树」 |
