...
2. **枝包含该节点**：`cutSeq` = 「该节点所属批次全部落定之后」的事件 seq，前缀 = `seq ≤ cutSeq`。
   ...
   **按块落地见 §12.13**（2026-09-22：块坐标 → 该块所在**步**的末尾；只给「消息」的调用方仍走整轮末尾）。
...
- **空间动作（P4-①，2026-09-19 定案并落地）**：从某个节点**拖出一条引线、松手落在纸上** = 就地立枝
  ...
  **判据 = 块 hover 才出现的「枝」小握把**（`.pp-branch-grip`：块**底间距带**内、右对齐，与动作行同一条带；
  来文块改落**块内尾带**——2026-09-19 悬停可达性批，见 §12.8 修订条；
...
| **P1′ 节点定位与入口** | ✅ **已落地**：投影锚点（`deriveMessageAnchors`）+ `resolveBranchPoint` + **消息动作行的「立枝」**（`use-block-ops`） | ① 来文块切点 = 该来文 seq、回复块切点 = **该块所在那一步的末尾**（2026-09-22 从「整轮末尾」修正——见 §12.13）；② 未落定/句柄缺席 ⇒ 具名拒绝；③ 中段切点立枝只到该节点为止；④ 入口位置守护（`[改,重发,抄,立枝]` / `[抄,立枝]`）——见 §12.2 |
...
- **握把落在块体之外的底间距带右端**（与动作行同带、各占一端）：块体之外 ⇒ 不吃划词选字；不是文类签
  ...
  **2026-09-19 修订（悬停可达性批，用户报「鼠标还没挪过去就消失」）**：块底 +2px 的缝是**死带**
  （hover 断链 ⇒ 隐没态 `pointer-events: none` 的握把/动作行**永不可命中**），且**来文块块下只有
  `userTailGap` 8px**（19px 的握把越界 15px 压住下一块）。现制：非来文块**顶边贴块底 + `::before`
  桥面**（视觉 2px 呼吸不变）；**来文块改落块内尾带**（花押之上 2px，块体之内 ⇒ 零死带零越界）。
  证据与弃案见 `paper-shell/taste-ledger.md` 2026-09-19 条 + 台架 `prototype/msg-ops-hover-ab.html`。
...
### 12.13 真机报的「立枝复制了一整个会话」——切点粒度从**整轮**修成**按块**（本批修）

**现象（用户真机，2026-09-22）**：在某条答复**中间**的块上点「立枝」，新枝卷里是**整卷**的内容
（用户原话：「好像不是从我选择的对应的块开的分支」「好像复制了一整个会话作为分支了」）。

**取证（真机数据，可重跑）**：工作区 `…\Documents\兰台\截图\.lantai\sessions`：

| 卷 | 盘上的事实 |
|---|---|
| 27 | 29 条事件、**只有一条来文**（一轮里 8 次工具调用、9 段正文）——即「整轮 = 整卷」 |
| 29 | 头行 `parent {id:27, atSeq:29}`——`29` 正是卷 27 的**最后一个** seq（切点落在整轮末尾） |

复现（合成同形卷 + 真定位链，改前）：一轮三步（每步「宣布调用 → 结果」）里**五个**界面块
（3 段正文 + 2 张工具卡）**全部**给出同一个 `atSeq` = 整轮末尾；改后同一批块给出
`4 / 4 / 6 / 6 / 7`（各自那一步的末尾）。

**机理（三件叠一起，缺一不成）**：

1. **入口在块上、定位在消息上**：动作行挂在**块**（`use-block-ops` 逐块建表），而
   `resolveBranchPoint` 只收「消息」——块与块之间的差别根本没进判定。
2. **实时形态里一条助手消息 = 整轮**：流式助手跨步复用同一条消息
   （`chat-stream` 的 `_finaliseStreamingAssistant` 直到下一轮 `TurnStarted` 才收尾），故整轮的
   正文段与工具卡都是同一条消息拆出来的块。
3. **旧语义是「回复块 → 整轮末尾」**（§12.2 的诚实边界 + §12.7 为「轮内内部来文不截断」加固过），
   于是点中间任意一块都切到整轮末尾——**单轮卷上，整轮末尾 = 整卷**。

**判性质**：不是读盘/落位/血缘的问题（那三面各有钉子），是**切点粒度过粗**——§3.2 白纸黑字写着
「枝包含该节点：`cutSeq` = **该节点所属批次全部落定之后**」，旧实现把「该节点」当成了「该消息」。

**修法（块坐标进定位链，末端多走一步）**：

| 件 | 内容 |
|---|---|
| `app/chat/session-branch.ts` | `BranchNode` 加 `key`（判据表的键 = **块** id）与 `block`（**块坐标** = 消息 `parts` + 本块下标）。答块分支改为：块坐标 → `placeablePartOf`（正文/工具卡自己有落点；资产卡/夹注取**最近邻块**）→ `stepIndexOf`（工具卡 = `tool_call_id` **身份匹配**；正文 = 本轮内内容相同的那条，同文本按块序消歧）→ `stepEndOf`（往后吃掉**紧邻**的工具结果）。定位不到 = 退回整轮（宁可切宽不切错：缺内容看不出来，多带内容看得见） |
| `app/chat/chat-core.ts` | `branchFromMessage(msg, sid, drop?, block?)`——**新增第四位 = 块坐标，两参数各自独立留在原位**：插件产物走磁盘通道热更（换产物不重编译 exe），而本函数住在壳里——产物已更、壳未重建时，块坐标若占了第三位，旧核会把它当**落点**（`drop.x` undefined ⇒ NaN 落位）。`branchPoints` 的节点键 = **块** id |
| `paper-shell/use-block-ops.ts` | 判据表按**块**建（键 = 块 id，节点带块坐标）；点击把块坐标一起交给 core |
| `paper-shell/use-branch-drag.ts` | 按下握把时记下本块的 `source.part`，松手时按它算块坐标——手势入口与按钮入口同一条判据 |

**粒度边界（诚实，两条都不越界）**：① 正文段与**同一条消息里**宣布的调用不可切在中间
（同一个 `assistant/text` 事件——§4 的硬边界）⇒ 正文块的切点 = **该步的结果落定之后**，
不是「正文这一段之后」；② 结果还没落（正在跑）⇒ 切点退回宣布处，由**同一条**落定校验具名拒绝
（不静默切到别处）。
...
| 件 | 内容 |
|---|---|
| `tests/session-branch.test.ts` +4 | ① **实时形态**（一条助手消息聚合整轮）：正文/工具卡各切各步（4 / 4 / 6 / 6 / 7）、收尾正文 = 它自己；同一用例并排断言**不带块坐标 = 整轮末尾（7）**（旧语义保留给消息级调用方）＋判据表按块键；② **重开形态**（一步一条消息）：同卷同判据（4 / 6 / 7）——两种形态一块尺子；③ **没有落点的块**（资产卡）：取最近邻块那一步；④ 这一块的调用**还没落定** ⇒ 具名拒绝，来文块照常可立枝 |
| `tests/paper-block-branch-op.test.tsx`（改写） | 判据表的节点 = **块**（键 + 块坐标）；点正文块 = `branchFromMessage(本块, 本卷, undefined, { parts, index: 0 })`、点**同一条消息的工具卡** = index 1；未落定的**那一块**置灰且不长握把，同消息的另一块照常 |

**门禁**：`npm run build` ✓；`npx vitest run` 371 文件 / **3863** 通过（受影响三文件改前 40 例绿、
改后 44 例绿；本批 +4 例）✓；`npx biome ci .` 0/0 ✓；`npm run doc-check` ✓。

**未动的地方（诚实边界）**：切点仍是「事件 seq」——不动事件词表、不动投影语义、不新增事件 kind
⇒ 零 BCR（`src-ui/src/agent/session-log.ts` 一行未改，故本批无需 convergence）；血缘/连坐/引线/
落位/卷名五条面一律未动。**测试面声明**（测试改造禁令）：本批在 `session-branch.test.ts` 只**新增**
用例（既有 32 例零改动、全绿——含「回复块 = 整轮末尾」那两条：无块坐标的调用方走的就是那条路，
语义未变）；`paper-block-branch-op.test.tsx` 的改写是**入口契约变了**（键从消息改成块、点击多带块
坐标），4 例的**原意**（位置护栏 / 同源判据 / 握把可见性 / 置灰不可点）逐条保留，属显式规格变更。
