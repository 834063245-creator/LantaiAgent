...
**版本节奏（2026-09-22 定）**：随包件**不自动跟随上游最新版**——pin 是刻意的，理由三条：
① 哈希校验要有意义，必须先知道指纹（"每次取最新 + 用上游自己给的指纹验自己"等于没验）；
② 兰台工具层与 CLI 动词耦合（只放行 11 个动词，且有漂移守护测试），盲目跟进上游改命令行
会**发版后**才炸在用户手里；③ 可重现（同 commit 任何时候打出同一个包）。
随包件另钉 `OFFICECLI_SKIP_UPDATE=1` ⇒ **它不会像用户显式安装位那样自更新漂移**。

代价（如实记）：本机装的是 1.0.152、随包 pin 的是 1.0.149 ⇒ **开发机比用户新 3 个补丁版**；
「我这儿好用、用户那儿报错」时先怀疑这个版本差。补的两个信号：
`node scripts/fetch-officecli.mjs --upstream` 手动查（**只提示**：不改 pin、不影响退出码），
以及 release.yml 里一条 `continue-on-error` 的提醒步（发版时可见，不阻断）。
