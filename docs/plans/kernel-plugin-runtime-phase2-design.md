...
- **P2-6 已落地**（2026-09-04 施工窗，全门禁绿）：builtin.pty 4 工具 + builtin.lsp 3 工具（生成器发射 manifest——pty/lsp 无模型面 TS zod 工具，schema 手写 spec 发射）。**权限形状按 §8.5 裁决**：无家族规则，不进 manifest permission——dispatch 侧 adapter 恒 Passthrough，插件走既有高层函数（pty_manager::pty_spawn/write/resize/kill、lsp_manager::lsp_start/request/stop，#[tauri::command] pub async 直接可调，AppHandle 经 ctx.app 传入）；生命周期注册表不暴露 ToolContext；pty-output / lsp-message 事件通道原样保留。rpc 52→45 methods（-7 分支）；rpc.rs 删 PTY/LSP 分区 + lsp_request result-shape 行 + 死码（req_u16/opt_u32）；TS：RpcContract 删 pty/lsp 行（EventContract pty-output/lsp-message 保留）+ kernelPtyCall/kernelLspCall/kernelLspRequest 信封助手；lsp-client.ts 七处 lsp_request + start/stop 换信封（lsp_request 由 JsonValue 自动展开改为 parseJson 显式恢复——kernelLspRequest 承载）；lsp-session 测试适配 toolCallArgsOf。Phase 2 全量竣工：九域全部信封化，rpc 45 methods。
- Phase 2 全量竣工（P2-0~P2-6 全部落地，见 §0 各行）。
