...
| `memory-domain` | ✅ **批 9h-4 已归家**（2026-09-26）：`agent/memory.ts` 733 + 随行 `agent/memory-bundle-client.ts` 134 两件 `git mv` 进 `plugins/builtin/memory-domain/` | ✅ 同批**形状上收 + 登记表门面**：新 `agent/memory-contract.ts`（`MemoryManagerFace` 覆盖内核装配面 + 产品工具工厂取用面；`MemoryImplementation` 两动词）+ `agent/memory-impl.ts`（门面 `createMemoryManager`/`memoryBundleIngest`，缺实现具名 fail-loud）；**事实保存授权旗标随迁入内核**（跨模块一次性状态留内核、产物经桥取用）；内核 `workspace.ts` / `context.ts` / `runtime/types.ts` / `agent-builder.ts` / `tool-rows.ts` / `chat-core.ts` 改走门面 |
| `skill-domain` | ✅ **批 9h-3 已归家**（2026-09-26）：`agent/skills.ts` 377 + `agent/builtin-skills.ts` 358（出厂技能内容）两件 `git mv` 进 `plugins/builtin/skill-domain/` | ✅ 同批**形状上收 + 登记表门面**：新 `agent/skill-contract.ts`（`SkillDef`/`SkillScan`/`SkillRegistryFace`/`SkillImplementation`）+ `agent/skill-impl.ts`（`createSkillRegistry`/`scanSkills`，缺实现具名 fail-loud）；内核 `workspace.ts` / `runtime.ts` / `skill-catalog.ts` / `settings-domain` 改走门面 |
| `task-domain` | ✅ **批 9h-5 已归家**（2026-09-26）：`agent/task.ts` 178 + `agent/task-board.ts` 319 + `agent/tools/board-status.ts` 78 三件 `git mv` 进 `plugins/builtin/task-domain/`（整包一次搬完——§3.1 实测：部分搬迁会让 `plugin-home-ledger` 的「包仍引用自己 impl」判据自相矛盾） | ✅ 同批**形状上收 + 登记表门面**：新 `agent/task-contract.ts`（`Task`/`TaskStatus`/`TaskManagerFace`/`BoardEntry`/`BoardStatus`/`TaskBoardFace`/`TaskBoardProxyFace`/`TaskBoardReadFace`/`TaskImplementation`）+ `agent/task-impl.ts`（五门面 `createTaskManager`/`createTaskBoard`/`createTaskBoardProxy`/`createTaskTools`/`createBoardStatusTool`，缺实现具名 `TASK_DOMAIN_UNAVAILABLE`）；余 `agent/board-persistence.ts` 121 **判内核共享面**（内核 `discovery-board.ts` 与包内 `task-board.ts` 共用 ⇒ 名册 `shared` 认领，不随包）；内核 `runtime.ts` / `workspace.ts` / `capability-segments` 改走门面 |
| `capability-segments` | ✅ **批 9h-1 已归家**（2026-09-26）：`agent/blueprint.ts` 410 按「内容 vs 机制」切开——十四项 capability 定义（+ 表内私有 helper `requireAgent` / `STATE_HOOKS_UNAVAILABLE`）**281 行进包** `plugins/builtin/capability-segments/segments.ts`（逐字搬移、**数组序原样** = 注册序 = 工具面表序） | 内核 158 行留形状与机制：`AgentCapability` / `BlueprintScope` / `BlueprintDeps` / `CapabilityPhase` + `AgentBlueprint` 类；桥面翻面成取用面 12 键（批 6/7 登记表读面 6 + 内核工具工厂/域折叠表 6） |
...
| `agent-loop-service` | ✅ **批 9h-2 已归家**（2026-09-26）：`agent/agent-loop/default-loop.ts` 469 整件 `git mv` 成 `plugins/builtin/agent-loop-service/default-loop.ts`——注册表与本实现在同包同装载；桥面翻面成循环依赖面 3 键（`finishReasonMessage` / `StreamingToolExecutor` / `typedRpcWithTimeout`） | ✅ 同批**内核去兜底**：`agent-loop-active.ts` 的 `resolveAgentLoop()` 无服务时具名 fail-loud（`AGENT_LOOP_UNAVAILABLE`），`agent.ts` 改查表；名册标 `required`（不可禁用）；**契约面 v50 → v51**（`AgentLoop` / `AgentLoopHost` 形状逐字未动） |
...
| 块 | 路径（物理行） | 备注 |
|---|---|---|
| Provider 控制台家族 8 件 | ~~`app/panels/settings/`：ProviderPage 721 · ProviderDetail 784 · AddProviderSheet 802 · ProviderList 85 · ProviderAdvanced 102 · ProviderDocCard 161 · protocol 35 · status 50 = **2,740**~~ | ✅ **批 9d 已归家**（2026-09-26）：8 件整组 `git mv` 进 `plugins/builtin/settings-domain/`；有状态内核依赖面走包内桥（+23 faceDeps 键：`createProvider` / `activeLlmAdapters` / catalog 五值 / credentials 三值 / oauth 四值 / `loadProvidersDoc` / settings 三值 / `mountDialogFocus` / `Overlay`），纯模块（types / vendor-templates / thinking / custom-headers）随包内联；名册 `impl` 8 条与 NOTES 条目一并销账 ⇒ 红区 9 → **8 产物 / 23 → 15 文件 / 7,478 → 4,738 行** |
...
| **产物私有排版引擎** | paper-shell 独占 5 件：~~`type-tokens.ts` 806~~ · ~~provenance 316 · sel-ink 138 · focus-flight 57 · sheet 36~~；compose-dock 独占 3 件：~~toc 275 · toc-ink 103 · ime 37~~ | ✅ **8 件全归家**：批 5a 收 7 件（962 行）；**批 9c-4b 收 `type-tokens.ts`**（807 行，与 `measure.ts` 2,016 同批随 `paper-shell`）——**账② 至此清零** |
| 拆分件 | ~~`measure.ts` **2015** · `group.ts` 306 · `virtualize.ts` 155 · `selection.ts` 193~~ | 类型/账本留内核，实现进 paper-shell | ✅ **批 9c 已落三件**（2026-09-26）：`selection.ts`（9c-1）· `virtualize.ts`（9c-2）· `group.ts`（9c-3）实现整件随包，形状分别上收内核契约 `paper/selection-contract.ts` / `region-geom-contract.ts` / `group-contract.ts`；faceDeps 291 → **279 键**（-12）。✅ **批 9c-4 全落**（2026-09-26）：9c-4a 接缝先行（`paper/measure-contract.ts` + `paper/measure-seam.ts`，`2268bd64`）；
  **9c-4b** `measure.ts` 2,016 + `type-tokens.ts` 807 整件随 `paper-shell`（`KERNEL_DEFAULT` 删除 ⇒ 缺实现具名
  fail-loud；名册标 `required`；宿主面 334 → **324 键**：撤 12 个 measure/type-tokens 键、补登记口 2 键）；
  **9c-4c** 测试面 29 件改指包内 + 真机验收（见 §6.5）。原口径**实测修正**：内核消费者不止 `clearObservedHeightsForSession` 一个，`paper/ink.ts` 还要 `inkSourcesFor` / `measureSignature`（墨迹走查 = 同一台 markdown 测高引擎）⇒ 切法须「墨迹路径 + 实测回写账留内核、块级测高/镜像常量/卷首/`injectPaperTokens` 随包」 |
...
**2026-09-26 基线**（批 9c-4b 后重测）：红 **0 产物 / 0 文件 / 0 行（账②清零）**；绿 **122 平台 + 121 已认领**；
...
ask 卡架 776 + Host 30 随包 ⇒ 灰区 61 → 58 文件 / 18,388 → 17,000 行；**9h-1 capability 内容表
随包** ⇒ 四个内核件转被认领（`agent/code-run/host` 269 · `agent/runtime/agent-builder` 195 ·
`agent/code-run/code-execution-tool` 105 · `agent/tools/board-status` 78）⇒ 灰区 58 → 54 文件 /
17,000 → 16,353 行、已认领 115 → 119；**9h-2 出厂默认 loop 随包** ⇒ 灰区 54 → **53 文件 /
15,711 行；**9h-3 技能域整件随包** ⇒ 灰区 53 → **52 文件 / 15,355 行**、已认领 119 → 120；**9h-4 记忆域整件随包** ⇒ 灰区 52 → **51 文件 / 15,224 行**、已认领 120 → 121；
**9c-4a 计量契约两件（`paper/measure-contract.ts` 49 + `paper/measure-seam.ts` 82）** ⇒ 灰区 51 → 53 文件 / 15,224 → 15,355 行；
**§4-6 `ctx.tokenMeter` 落地** ⇒ 内核 service 16 个、清单 54 条，`agent/token-meter/` 由「§4-6 B 白名单」改为
「§4-6 A 内核 service」口径（平台 120 → 122：该前缀下新增 `contract.ts` / `service.ts` 两件），**灰区与红区零变化**——本批是分类收口不是搬迁）。
**9h-5 `task-domain` 整件随包**（2026-09-26）⇒ 红区 2 产物 / 5 文件 / 1,502 行 → **1 产物 / 1 文件 / 806 行**
；**9c-4b 测量引擎随 `paper-shell`** ⇒ 红区 1 → **0**（`type-tokens` 806 销账、`measure` 2,016 随迁）、
灰区 52 → **50 文件 / 15,103 行**（`paper/measure-contract.ts` + `paper/measure-seam.ts` 两件转被认领）。
平台 122 不变、已认领 121 不变（9h-5 三件实现出内核、两个新契约件进内核恰好抵平；9c-4b 两件出内核、两件转被认领亦然）。
**2026-09-26 纯壳集清零 + 账② 清零**：账本 §1 立账 21 条（18 纯壳 + 3 半壳）全部销完，`impl` 名册字段全空
——「实现仍留内核」这一整本账**归零**，此后新欠账只可能以「新造空壳未登记」的形态出现（守卫钉住）。
...
> **§4 争议项裁定表（2026-09-25 用户拍板，比本页 §4 新）**：
...
- **9d Provider 控制台归家**（`9868a5f4`，2026-09-26）：8 件 2,740 行（ProviderPage 721 ·
  ProviderDetail 784 · AddProviderSheet 802 · ProviderList 85 · ProviderAdvanced 102 ·
  ProviderDocCard 161 · protocol 35 · status 50）整组 `git mv` 进 `settings-domain/`；
  桥面按「有状态走桥、纯模块内联」切（+23 faceDeps 键 ⇒ 279 → **297**，撤 `ProviderPage` 键；
  纯模块 provider/types · vendor-templates · thinking · custom-headers 随包内联，不制造第二份状态）；
  名册 `impl` 8 条 + NOTES 条目销账（半壳只剩 paper-shell）⇒ 红区 **9 → 8 产物 / 23 → 15 文件 /
...
- **批 10 部件一 + 部件三：两条宿主生命周期贡献面**（2026-09-26，设计件 [`workspace-activation-channel-design.md`](workspace-activation-channel-design.md) §4.1/§9；用户 2026-09-25 裁定 A）：
  ...
  （`ctx.shellRows`，第 18 service）——`bootShell` 在内核行之后**追加**贡献行（注册序）；首消费者
  `shell/rows/update-check.ts`（27 行）随 `settings-domain` 包（内核壳行 **11 → 10** + 贡献行 1 ⇒ 引导序逐位复现）。
  **契约 v52 → v53**（两条载体同一次升版）；内核 service 16 → **18**、第一方清单 54 → **56**；宿主面**零新键**
  （327 / `28da6bf5` 不变）。守卫 = `tests/host-lifecycle-channels.test.ts`（串行序 / 失败隔离 / disposer 摘行 /
  无服务=空面 / 贡献行追加末位）；`privileged-zone-freeze` 基线 32 → **34**（两条通道本体入区，裁定 A 授权）。
  **对账**：红 0 · 平台 122 → **123**（实测）· 已认领 120 · 灰 50 / 15,103。
  **真机验收**（重建 exe + CDP，2026-09-26）：启动期 57 条 fiber 全 `[active]`（`settings-domain`
  `apply=2.4ms`）、`[boot-timing] auditBoot settle：57 条 fiber 整树 0.1ms`、**零装载失败/零壳行 boot 失败**；
  宿主面运行时 **327 键**（两条通道零新 faceDeps 键，`workspaces` / `shellRows` 均不在面内 ✓ 内核专用）；
  进工作区一次通过（`.pp-root` + 卷首题字）——即**工作区接线通道真路径**零异常；
...
- **9f-1 Provider 编辑面随 `settings-domain`**（2026-09-26，施工单 [`provider-data-face-homing-design.md`](provider-data-face-homing-design.md)）：
  ...
  `ProviderDetail.tsx` / `ProviderPage.tsx` / `SettingsPanel.tsx` 此前**直接 import 内核 `settings.ts` 取值**
  ⇒ 产物域把整个 `settings.ts`（含模块状态：保存订阅表 / provider 投影）内联成副本（§0.6 同族）——现改指
  包内件与 `./host` 桥（有状态内核模块一律经桥，纯函数件直连并注明）。宿主面 324 → **327 键**（撤 4 补 7，
  指纹 `28da6bf5`）。**9f-2/9f-3**：`provider/**` 15 件判内核平台面（名册 `shared` 认领）与真机验收。
  三色：红 **0** · 绿 122 平台 + **120 已认领** · 灰 50 / 15,103。
  **门禁**：build ✅ · vitest **407 文件 / 4,308 项** ✅ · biome 0/0（2 存量 info）✅ · convergence 双轨零漂移 ✅ ·
  doc-sync / doc-check ✅ · build:builtin-plugins / gen:host-surface ✅（327 键）。
  **真机验收**（重建 exe + CDP，2026-09-26）：宿主面运行时 **327 键**（= 基线键集 = 指纹 `28da6bf5`），
  撤的 4 键确认消失、补的 7 键类型全对；装载日志 `hologram/settings-domain manifest=144ms face=144ms
  import=265ms apply=1.2ms [active]`；`entry.js` **832,928 B** 含 `isFactoryBaseUrl` / `persistSecrets` /
...
- **9f 侦察（2026-09-26，未施工——下次从这条接）**：`settings.ts` + `workspace.ts` **都不是单文件搬迁**。
  ...
  **provider 数据面归家**，需一份施工单（下一轮先出设计件再动刀）。
  ...
  编辑面 ≈220 = −293**（原估 −700 偏大）。子批 9f-1（搬 + 销键）/ 9f-2（名册 `shared` + 账目）/
  9f-3（真机验收）。
...
- **9h-1 capability 内容表随包**（`2ae54739`，2026-09-26）：`agent/blueprint.ts` 410 按「内容 vs
...
- **9h-2 出厂默认 loop 随包**（`9eccc861`，2026-09-26）：`agent/agent-loop/default-loop.ts` 469
  整件 `git mv` 进 `plugins/builtin/agent-loop-service/`（与注册表同包同装载，原文件不再存在）；
  内核 `agent-loop-active.ts` **去兜底** —— `resolveAgentLoop()` 无服务时具名 fail-loud
  （`AGENT_LOOP_UNAVAILABLE`），`agent.ts:556` 改 `opts.agentLoop ?? resolveAgentLoop()`；名册标
  `required`（不可禁用）。桥面翻面成循环依赖面 3 键（`finishReasonMessage` /
  `StreamingToolExecutor` / `typedRpcWithTimeout`）⇒ 宿主面 321 → **323 键**（指纹 `faade5dd`）。
  **契约面四步流程**：清单行改指产物路径 + `OPEN_SURFACE_CONTRACT_VERSION` 50 → **51** +
  载体表行/变更记录行 + `gen:contract-fingerprint`（18 文件，指纹 `25e379fa…`）——
  `AgentLoop` / `AgentLoopHost` 形状逐字未动。测试面：「无服务回落默认」按行为退役改写成
  fail-loud 断言 + 「服务在场取注册表实现」；`tests/setup.ts` 在内核根 Context 补挂该产物
  ...
  **14.5 KB** 动态 import 成功，含 `builtin/default` / `AgentLoopService` / 默认 loop 的
  ...
  零装载失败、零异常。
- **9h-3 技能域整件随包**（2026-09-26）：`agent/skills.ts` 377 + `agent/builtin-skills.ts` 358 两件
  `git mv` 进 `plugins/builtin/skill-domain/`。**契约面留内核**：新 `agent/skill-contract.ts`
  （`SkillDef` / `SkillScan` 逐字上收 + `SkillRegistryFace` 结构面 + `SkillImplementation` 三动词）；
  **登记表留内核**：新 `agent/skill-impl.ts`（`register/active/clear` + 门面
  `createSkillRegistry` / `scanSkills`；缺实现 = 具名 fail-loud `SKILL_DOMAIN_UNAVAILABLE`）。
  内核调用点改查表：`workspace.ts`（`new SkillRegistry` → `createSkillRegistry()`，字段类型改面）·
  `runtime.ts` 装配期扫描（调用点既有 catch 保住「读不到不炸装配」）· `app/commands/skill-catalog.ts`
  动态 import · `settings-domain/host.ts` 的 SkillsPage 列表源；`agent-builder.ts` / `tool-rows.ts`
  类型改指契约。桥面四键（`kernelGlobalMemoryDir` / `kernelListDirectoryFlat` / `kernelReadFile` /
  `defineTool`；`yaml` / `zod` 裸包随产物整包内联）⇒ 宿主面 323 → **324 键**（指纹 `e6e03064`）。
  名册 `impl` 销账（空壳集 3 → **2 条 = 1 纯壳 + 1 半壳**）。红区 4 → **3 产物 / 3,104 →
  2,369 行**；灰区 53 → **52 文件 / 15,711 → 15,355 行**；已认领 119 → **120**。行为零变更
  （convergence 双轨零漂移为证）。
  **真机验收**（重建 exe + CDP，2026-09-26）：宿主面运行时 **324 键**（= 基线键集 = 指纹
  `e6e03064`）、四个新桥键类型全对且 `createSkillTool` 键已撤；`skill-domain/entry.js`
  **763 KB**（781,128 B——`yaml` + `zod` 随产物整包内联）动态 import 成功，含 `SkillRegistry` /
  `scanSkills` / 出厂技能内容 / `SKILL.md` 解析面真身；启动期 console 与前一批逐条相同，
  零装载失败、零异常。
- **9h-4 记忆域整件随包**（2026-09-26）：`agent/memory.ts` 733 + `agent/memory-bundle-client.ts` 134
  两件 `git mv` 进 `plugins/builtin/memory-domain/`。**契约面留内核**：新 `agent/memory-contract.ts`
  （`MemoryEntry` / `MemoryFile` / `MemorySavedInfo` 逐字上收 + `MemoryManagerFace`——内核装配面用点
  `onSaved` / `loadPromptSection` **加**产品工具工厂取用面 `list` / `read` / `save` / `delete` /
  `loadIndexText` / `scopes`，因工具行材料经 `composition/tool-rows.ts` 把同一实例交给产物包）；
  **登记表留内核**：新 `agent/memory-impl.ts`（门面 `createMemoryManager` / `memoryBundleIngest`，
  缺实现具名 fail-loud）——**事实保存授权旗标随迁入内核**（`authorizeFactSave` /
  `consumeFactAuthorization`：跨模块一次性状态留内核、产物经宿主桥取用）。内核调用点改查表：
  `workspace.ts`（`new MemoryManager` → 门面；`memoryBundleIngest` 改指门面）· `context.ts` /
  `runtime/types.ts` / `agent-builder.ts` / `tool-rows.ts` 类型 · `chat-core.ts` 的 `/remember` 授权
  改静态 import（原动态 `import('../../agent/memory.js')`）。桥面六值 ⇒ 宿主面 324 → **325 键**
  （指纹 `2578f632`）。名册 `impl` 销账（空壳集 2 → **1 条 = 0 纯壳 + 1 半壳**）。红区 3 →
  **2 产物 / 2,369 → 1,502 行**；灰区 52 → **51 文件 / 15,355 → 15,224 行**；已认领 120 → **121**。
  行为零变更（convergence 双轨零漂移为证）。
  **真机验收**（重建 exe + CDP，2026-09-26）：宿主面运行时 **325 键**（= 基线键集 = 指纹
  `2578f632`）、七个探针键类型全对且 `createMemoryTools` 键已撤；`memory-domain/entry.js`
  **555 KB**（568,541 B）动态 import 成功，含 `hologram_memory_save` / `hologram_memory_delete` /
  `MemoryManager` / `MEMORY.md` 索引面 / 记忆束 `/ingest` 真身；启动期 console 与前一批逐条相同，
  零装载失败、零异常。

- **§4-6 token 计量升为内核第 16 个 service `ctx.tokenMeter`**（2026-09-26，用户 2026-09-25 裁定 A）：
  新增契约载体 `agent/token-meter/contract.ts`（`TokenLedger` 每卷账本形状 · `TokenAlgebra` 分桶代数
  形状 · `TokenMeterImplementation` 实现面 · `types.ts` 六个读数类型在此重出口）+ 服务本体
  `agent/token-meter/service.ts`（`TokenMeterService extends Service`，`createLedger()` /
  `restoreLedger(snapshot)` / `usage`；模块级 `requireTokenMeter()` 缺服务**具名 fail-loud**
  `TOKEN_METER_UNAVAILABLE`，测试复位面 `resetTokenMeterForTests()`）。`meter.ts` 的
  `SessionTokenMeter` 挂 `implements TokenLedger`。**内核读点改造**：`agent/agent.ts` 三处
  （字段初值 / `restoreTokenLedger` / 新会话归零）不再 `new SessionTokenMeter()`，改
  `requireTokenMeter().createLedger()/restoreLedger()`；**录入点仍唯一 = `Agent.streamOnce`**
  （服务不含任何写账动作）。**装载面**：`plugins/service-plugins.ts` 表序**追加末位**
  （内核 service 先于产物装载，表序 = 字节契约不动）⇒ 内核 service 15 → **16**、清单 53 → **54**；
  `tests/setup.ts` 复现装载态。**契约四步**：v51 → **v52**（清单新增 `contract.ts`，
  19 文件指纹重录）。**消费者改道**：创作坞墨量册（`compose-dock/InkLedger.tsx`）此前直接
  import `agent/token-meter/{types,usage}`（产物域 esbuild 会内联成第二份副本）⇒ 改经包内宿主桥
  （`host.ts` 重出 `tokenAlgebra` + `TokenMeasurement` 类型），宿主面 329 → **330 键**
  （指纹 `d173a7ea`）。**口径零漂移**（四桶恒等于 `prompt_tokens` / 压力只算 prompt 侧 /
  部分命中不四舍五入）——`tests/token-meter.test.ts` 16 项**零改动**通过。红区/灰区**零变化**
  （本批是分类收口；灰区 53 文件 / 15,355 行的两件增量来自 9c-4a 的 `measure-contract.ts` /
  `measure-seam.ts`）。三色：红 2 产物 / 5 文件 / 1,502 行 · 绿 **122 平台 + 121 已认领** ·
  灰 53 文件 / 15,355 行。
  **真机验收**（重建 exe + CDP，2026-09-26）：宿主面运行时 **330 键**（= 基线键集 = 指纹
  `d173a7ea`），`faceDeps.tokenAlgebra` 在场且代数可算（`formatTokens(12345)` = `12.3k` ·
  `formatExactTokens(1234567)` = `1,234,567` · 四桶输入 16 / 总量 23 · 命中率文本 `95`）；
  进工作区（恢复卷「测试，看看你手里有没有图谱工具」）**一次通过**：`[Workspace.open] step 1:
  workspace_activate… / all done`、`.pp-root` 在场、坞与墨量册触发器在场（空账本态「墨 —」），
  **零** `TOKEN_METER_UNAVAILABLE`、零 `setupAgent failed`——即「Agent 构造经
  `requireTokenMeter().createLedger()`」「每卷账本恢复经 `restoreLedger()`」两条真机路径都活着。

- **9h-5 task 域整件随包**（2026-09-26，账② 收口）：`agent/task.ts` 178 · `agent/task-board.ts` 319 ·
  `agent/tools/board-status.ts` 78 三件 `git mv` 进 `plugins/builtin/task-domain/`（**整包一次搬完**——
  §3.1 实测：部分搬迁让 `plugin-home-ledger` 的「包仍引用自己 impl」判据自相矛盾）。**形状上收内核**：
  新 `agent/task-contract.ts`（`Task` / `TaskStatus` / `TaskManagerFace` / `BoardEntry` / `BoardStatus` /
  `TaskBoardFace` / `TaskBoardProxyFace` / `TaskBoardReadFace` / `TaskImplementation`）+ `agent/task-impl.ts`
  （五门面 `createTaskManager` / `createTaskBoard` / `createTaskBoardProxy` / `createTaskTools` /
  `createBoardStatusTool`，缺实现具名 `TASK_DOMAIN_UNAVAILABLE`）。**余 `agent/board-persistence.ts` 121
  判内核共享面**（内核 `discovery-board.ts` 与包内 `task-board.ts` 共用；随包会让内核反向依赖产物源码
  ⇒ 批 8a 守卫钉死）⇒ 名册 `shared` 认领。内核读点改造：`runtime.ts`（`new TaskBoard` /
  `new TaskBoardProxy` → 门面）· `workspace.ts`（`new TaskManager` → 门面）· `capability-segments`
  （`new TaskManager` → `createTaskManager` 门面，`TaskManager` 类不再跨包暴露）· 八处类型读点指契约
  （`agent.ts` / `context.ts` / `runtime/types.ts` / `agent-builder.ts` / `blueprint.ts` /
  `state-hooks-contract.ts` / `subagent-runtime-contract.ts` / `composition/tool-rows.ts` /
  `ui/agent-panel-store.ts`）。宿主面 330 → **334 键**（指纹 `2b4cd938`：+`BoardPersistence` /
  `registerTaskImplementation` / `clearTaskImplementation` / `spillToFile`，−`TaskManager` 类键）；
  `task-domain/entry.js` **559 KB**（三件实现类 + 两族工具真身）。名册 `impl` 清空（**纯壳集清零**：
  立账 21 条全部销完），`tests/plugin-home-ledger.test.ts` 口径随之改成 0 并删该条 NOTES。
  **两条新纪律（写入设计件 §3.1.1）**：① 三张域登记表（skill/memory/task）改**栈式**
  （register push / clear pop / active 读栈顶）——装配腰 dispose 贡献者 fiber 会撤销登记，单值登记
  会让「腰跑完之后」的装配撞 fail-loud（四个 composition 测试文件实测红）；② 登记口在
  `tests/setup.ts`（beforeAll + **beforeEach 重新断言**）与 convergence 夹具
  （`ensureTaskDomainRegistered()`，独立入口）**两处**都要有。另修 `product-source-not-in-bundle`
  的探针判据（跨行撇号会凑出 `).describe(` 一类**通用代码文本** ⇒ 假红；现要求探针是内容串，
  不含代码标点）。红区 2 产物 / 5 文件 / 1,502 行 → **1 产物 / 1 文件 / 806 行**；灰区 53 →
  **52 文件 / 15,234 行**；已认领 121 不变。
  **真机验收**（重建 exe + CDP，2026-09-26）：宿主面运行时 **334 键**（= 基线键集 = 指纹
  `2b4cd938`），八个探针键类型全对且 `TaskManager` 类键已撤；启动期装载日志
  `hologram/task-domain manifest=125ms face=125ms import=895ms apply=0.9ms [active]`（磁盘通道装载）；
  `task-domain/face.json` 需求键 = 桥面八键、`hostApi` = 当前基线；`entry.js` 558,726 B 含
  `TaskManager` / `TaskBoard` / `TaskBoardProxy` / `task_create` / `agent_board` /
...
- **9c-4 测量引擎整件随包（账② 清零）**（2026-09-26，施工单 [`measure-engine-seam-design.md`](measure-engine-seam-design.md) §4）：
  **9c-4a**（`2268bd64`）接缝先行——新 `paper/measure-contract.ts`（内核真读三动词 + `InkSource` 形状）+ `paper/measure-seam.ts`
  ...
  `PAPER_MEASURE_UNAVAILABLE`；名册标 **`required: true`**（引擎缺席 = 纸面高度全崩）+ `impl` 清空（**红区 0**）+
  `shared` 撤 `paper/measure.ts` 一条；登记表照 9h-5 的栈式纪律（register push / clear pop）。**宿主面 334 → 324 键**
  （指纹 `29f87402`）：撤 12 个 measure/type-tokens 键（`clearPaperMeasureCache` / `createBlockMeasureCache` /
  `measureBlockHeightCached` / `measureFolioHeadHeight` / `MARGINALIA_TOP` / `needsObservedHeight` / `observedKeyOf` /
  `reportObservedBlockHeight` / `reportObservedSidecarExtent` / `subscribeObservedBlockHeights` / `USER_SHRINK_MIN_W` /
  `injectPaperTokens`——包内组件直接 `from './measure'` / `from './type-tokens'`）、补登记口 2 键。**9c-4c**：测试面
  29 件 + `scripts/` 改指包内；`paper-visual-decisions` 的源文件断言点随迁。**包内新件**：`measure-implementation.ts`
  （三动词按契约面组装）+ `host.ts`/`host.aliased.ts` 桥面翻面为登记口。红区 1 → **0**；灰区 52 → **50 文件 /
  15,103 行**；平台 122 / 已认领 121 不变。
  **真机验收**（重建 exe + CDP，2026-09-26）：宿主面运行时 **324 键**（= 基线键集 = 指纹 `29f87402`），
  `registerMeasureImplementation` / `clearMeasureImplementation` 在场、12 个旧键已撤；装载日志
  `hologram/paper-shell manifest=43ms face=43ms import=269ms apply=2.0ms [active]`；`entry.js` **475,128 B** 含
  `measureBlockHeightCached` / `measureFolioHeadHeight` / `observedKeyOf` / `injectPaperTokens` / `FOLIO_TOKENS` /
  `registerMeasureImplementation` 真身；`face.json` 需求键 = 新桥面、`hostApi` = 当前基线；进工作区纸面渲染在场
  （`.pp-root` + 卷首题字 + 4 块），**零** `PAPER_MEASURE_UNAVAILABLE`——即「墨迹走查 → 接缝 → 包内引擎」真路径活着。
  **性能对拍**（设计件 §5 的判据）：`tests/perf-paper-pan.test.tsx` = `commits=60 avg=4.07ms p95=4.79ms max=5.79ms
  fillRect=0`（60 帧纯平移、小地图墨迹零重画）——门面只做一次模块级解引用，热路径无可测回退。

### 6.1 批 4c 施工侦察（`coding.ts` 五族拆分，2026-09-24 实测，下一轮直接用）**件**：`agent/tools/coding.ts` **998** + 随行私有件 `git-porcelain.ts` 126 · `sticky-cwd.ts` 138 ·
...
>
> **⚠ 读上表前先看这一条（2026-09-25 用户复议，比上表新）**：§4-6 由 **B 改判 A**
> ——立第 16 个内核 service `ctx.tokenMeter`（**2026-09-26 已落**，见 §6.5 + 契约 v52）；
> §4-9 由 B 改判 **A**（立壳行通道，与批 10 同窗）；§4-11 B（暂）**维持**但已改为
> 「先真机验收 → 批 10 一并产物化」（验收 2026-09-25 四条全通）。**权威表 =
> [`plugin-extraction-decisions.md`](plugin-extraction-decisions.md)**，本页下表仅存历史推荐理由。
