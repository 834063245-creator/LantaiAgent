...
**排序（2026-08-20 拍板：三阶段串行，用户带宽优先）：**

- **阶段 1 = 组合层核心**：S0 → S1 → S2。用户角色 = 批准 S1 设计件（已预写，约半小时）+ 看绿灯放行 commit——零深度参与，门禁替你看。
- **阶段 2 = 白纸全程**（`../paper-shell/` V0→V5）：agent 全部产能投前端；用户注意力集中于此，但形态是短会话（访谈 / 选型 / 打磨判读），不是巨理解。V3b 需要的本计划 S1 届时早已完成。
- **阶段 3 = 组合层收尾**：S3 → S4。**S3 排在纸方向确定之后不是让步，是本计划自身纪律的推论**——S3 协作纪律要求「重构推到哪个域，行化跟到哪个域」；先迁行再定纸方向，等于给可能被纸废除的面做迁移（返工）。S4（分发 / preset / hello 示例）无前置依赖，纯粹排后。
- 上午稿的并行拓扑优化的是日历吞吐，但本项目的稀缺资源是用户带宽——一人一 agent 的运营里，串行化人的注意力是正确解。技术上全程兼容：将来若带宽充裕想并行，把阶段 3 提前即可，无返工。
