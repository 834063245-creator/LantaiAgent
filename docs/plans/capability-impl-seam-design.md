...
### 6d compaction（= 2,030 行：1,059 + 644 + 327）—— ✅ **已全落（6d-1 + 6d-2）**

**6d-1（2026-09-24，结构切分，commit `a6fd4d9b`）**：记账面切出 → `agent/compaction-tracker.ts`；
契约面上收 → `agent/compaction-contract.ts`；`agent-loop/types.ts` 一行类型导入改指 ⇒ 开放面契约
47 → **48**（四步流程走完）。

**6d-2（2026-09-24 落地实况）**：`agent-compaction.ts`（1016）· `compaction-model.ts`（425，策略面）·
`compaction-summarize.ts`（332）三件 git mv 进 `plugins/builtin/compaction/` + 包内壳 166 行；
契约面补 `CompactionConfig` / `SummaryCall` / `SummaryRun` / 两个比例默认 / **`CompactionImplementation`
（16 项，`runCompaction` 只包内调用故不入面）**；内核留 414 行（tracker 206 / contract 156 / impl 52）。
`Agent` 15 处 + `agent-builder` 1 处改 `requireCompactionImplementation().xxx(...)`（service 语义：
调用点 fail-loud）。faceDeps **+19 键**，指纹 `9eb85fdc → 5a542fb9`；34 产物全部带 face.json。

**实测教训（写给后续批次）**：常驻登记腰**不能**收进 `tests/setup.ts`——全局 setup 先于测试文件的
`vi.mock` 提升执行，预载内核模块会架空 mock（实测 `plan-outcome` 等全线红）；正确形态 = 每个需要的
测试文件显式 import 腰（in-band，与测试文件同受 hoisting 管辖）。
