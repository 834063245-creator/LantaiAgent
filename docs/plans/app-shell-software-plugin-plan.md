...
> **S0-S6 全竣工**——S1 数据目录 / S2 受治治理 / S3 窗口原语 / S4 唤醒回调 /
> S5 notes-app 范本 / S6 文档收口已落地（同日五窗连推，门禁全绿）；转归档
> 待用户真机验收 + 管理 UI 面（设计定稿后另批）**
...
- **竣工（2026-09-06）**：落地面——`docs/plugins/README.md` **§10 软件级插件
  （app shell 四件套）**（目录新条目 + §2 manifest 节补软件级字段指针 + 责任
  切分（协议面归插件——纯 GUI 软件允许装载不进主路）+ 四件套 × manifest
  字段对照表（A 窗口原语/B 数据目录/C 受治进程/D 后台唤醒——每件「宿主给
  什么」一句话钉死）+ 两张门规则 + 装载/开窗/关窗/卸载语义链 + 契约版本
  v14-v17 指针；§9 未决项补「管理 UI 面待用户设计定稿」如实声明）；
  **新 cookbook `docs/cookbook/plugin-as-software.md`**（五步指南：manifest
  骨架 → 窗内容与桥 SDK → 数据地盘 → 受治 server 与三档策略表 → 两张门
  各放什么 → 异步长任务双路写法 → 端到端验证与复制清单——notes-app 全程
  示例）；`docs/README.md` cookbook 索引行增补；`docs/plans/README.md` 计划
  入口登记翻为「S0-S6 竣工，余管理 UI 面（设计定稿后另批）+ 用户真机验收」。
  **转归档暂缓**（如实）：归档以用户真机验收（S5 清单：装载 → 开窗 →
  notes_create 窗内可见 → notes_export 异步唤醒 → 关窗空闲回收 → 卸载
  .trash）+ 管理 UI 面子件（设计定稿后另批施工）为前置——本计划保留
  In progress 至验收过再挪 `docs/archive/`。
