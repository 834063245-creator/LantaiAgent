...
1. 科研回答流式输出含 `$$...$$` 公式：渲染为排版公式，流式半程不破版、finalised 后不闪
2. 行内 `$...$`（如 $E=mc^2$）在中文正文混排正常、不误伤货币/普通 `$`
3. Agent 调 show_asset 出引用卡：DOI/arXiv 可点开、BibTeX 折叠、历史卡只读
4. 公式块/引用卡钉住（pinned）+ 折叠态下行为正常、无叠字空跳
5. 旧卷回放（历史会话含科研内容）过新管线渲染正常
