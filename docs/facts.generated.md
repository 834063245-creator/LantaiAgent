...
| `factory_products` | 出厂产物数（builtin-roster.json） | **33** | `src-ui/src/plugins/builtin-roster.json` |
| `tool_domains` | 兰台应用侧域工具数（src-ui DOMAIN_SPECS） | **11** | `src-ui/src/agent/tools/domains.ts` |
| `open_surface_contract_version` | 开放面契约版本 | **48** | `src-ui/src/composition/contract-version.ts` |
...
| `first_party_plugins` | 第一方插件总数（内核 + 出厂产物） | **46** | `src-ui/src/plugins/loader.ts + src-ui/src/plugins/builtin-roster.json` |
