...
| `builtin_service_plugins` | 内核插件数（BUILTIN_PLUGINS 表） | **15** | `src-ui/src/plugins/service-plugins.ts` |
| `factory_products` | 出厂产物数（builtin-roster.json） | **38** | `src-ui/src/plugins/builtin-roster.json` |
| `tool_domains` | 兰台应用侧域工具数（src-ui DOMAIN_SPECS） | **11** | `src-ui/src/agent/tools/domains.ts` |
| `open_surface_contract_version` | 开放面契约版本 | **51** | `src-ui/src/composition/contract-version.ts` |
| `engine_contract_version` | 引擎开放面契约版本 | **9** | `engine/src/contract.rs` |
...
| `first_party_plugins` | 第一方插件总数（内核 + 出厂产物） | **53** | `src-ui/src/plugins/service-plugins.ts + src-ui/src/plugins/builtin-roster.json` |
