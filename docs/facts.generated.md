...
| `builtin_service_plugins` | 内核插件数（BUILTIN_PLUGINS 表） | **16** | `src-ui/src/plugins/service-plugins.ts` |
...
| `open_surface_contract_version` | 开放面契约版本 | **52** | `src-ui/src/composition/contract-version.ts` |
...
| `first_party_plugins` | 第一方插件总数（内核 + 出厂产物） | **54** | `src-ui/src/plugins/service-plugins.ts + src-ui/src/plugins/builtin-roster.json` |
