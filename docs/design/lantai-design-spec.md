...
| 浮层 | 引/思考/斜杠/翰律/模型菜单 | 45-90 | `--elev-menu` | 必须盖住内容 |
| 模态 | 设置/命令面板/媒体预览 | veil + 400/401/520 | `--elev-menu` + veil | veil 必配（媒体预览加墨幕） |

规则：① 一个光源方向永远（letterpress `2px 3px`，左上光，方向乱则层次死）② 浮层必须盖住内容才配海拔——什么都不盖的回地面 ③ 模态必配 veil（「下面还在但退后了」）④ 影随档位增强，档间差必须可见。
