...
兰台内置的 50 个第一方插件（**14 内核 service + 36 出厂产物**）与第三方走同一套通道。
内核 14 件编译进 exe（注册表/运行时 + code-runtime + dynamic-runner + lsp-service——真源 =
`src-ui/src/plugins/service-plugins.ts`，loader 的 `BUILTIN_PLUGINS` 由它派生）；出厂产物 36 件真源在 `plugins/builtin/<name>/`
