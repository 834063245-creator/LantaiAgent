...
describe('composition/prompt-sections（S1-4 section 注册表，B④ 收官纯插件面）', () => {
  it('section id 唯一且稳定：第一方面 8 段（清单序 = 迁移前出厂表序 − graph-snapshot）', () => {
  ...
  it('两面互斥分流：零目录只剩 brief 段；有目录时 brief 段全跳过', () => {
    ...
    const full = { memorySection: 'mem', claudeMdSection: 'md', shellEnvSection: 'env-line' };
