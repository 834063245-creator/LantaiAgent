...
describe('edit/rename 透传 _agent_id（worktree 路由）', () => {
  it('edit_file 保留 _agent_id', async () => {
    ...
    // R4-4b 起 edit 动作经 editor_cap 直呼（builtin.editor 信封退役）——
    // 守护点不变：_agent_id 必须原样透传（worktree 路由）。
    expect(calls[0].name).toBe('editor_cap');
    const env = calls[0].args as { action?: string; file_path?: string; _agent_id?: string };
    expect(env.action).toBe('edit_file');
    expect(env.file_path).toBe('D:/p/a.ts');
  ...
  it('fs(edit) 领域工具路径同样保留 _agent_id', async () => {
    ...
    expect(calls).toHaveLength(1);
    // R4-4b 直呼后守护点不变：action=edit_file 且 _agent_id 原样透传。
    expect(calls[0].name).toBe('editor_cap');
    const env = calls[0].args as { action?: string; file_path?: string; _agent_id?: string };
    expect(env.action).toBe('edit_file');
    expect(env.file_path).toBe('D:/p/a.ts');
...
describe('R13 所有权不被领域工具绕过', () => {
  it('两个 fresh 子 Agent 并发 fs(edit) 同一文件：先写者落盘，后写者收到 [已拒绝]', async () => {
    ...
    const exec: ToolExecutor = async (name, args) => {
      // R4-4b 起 fs(edit) 经 editor_cap 直呼（builtin.editor 信封退役）。
      const cap = name === 'editor_cap' ? (args as { file_path?: string }) : null;
      if (cap) {
        editCalls.push(cap.file_path ?? '');
