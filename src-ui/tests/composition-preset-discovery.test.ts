...
describe('composition/preset-discovery（S4-0 用户层发现）', () => {
  ...
  it('正常发现：索引 + 本体 + 元数据 → roster 合并（内置在前，用户按 id 序）', async () => {
    await discoverPresets({
      origin: ORIGIN,
      fetchImpl: router({
        [ORIGIN + '/']: { status: 200, body: '["paper", "alpha"]' },
        [ORIGIN + '/paper/roster.patch.yml']: { status: 200, body: PAPER_PATCH },
        [ORIGIN + '/paper/preset.yml']: { status: 200, body: 'name: 纸壳\norder: 2\n' },
    ...
    expect(roster.map((p) => p.id)).toEqual(['standard', 'minimal', 'alpha', 'paper']);
    const paper = roster.find((p) => p.id === 'paper');
    expect(paper?.builtin).toBe(false);
    expect(paper?.patch).toEqual({ tools: [{ id: 'builtin/web', disabled: true }] });
    expect(paper?.metadata).toEqual({ name: '纸壳', order: 2 });
