...
import { builtinPromptSections } from '../src/composition/prompt-sections';
import {
...
describe('composition/roster（S2-0 组合引擎）', () => {
    ...
    const layers = [
      {
        ...
        prompt: [{ id: 'behavior-rules', text: 'X' }],
      },
    ];
  ...
  it('disable：prompt 段 / capability / shell 同语义', () => {
    const r = resolveRoster(factoryComposition(), [
      {
        prompt: [{ id: 'multi-agent', disabled: true }],
        capabilities: [{ id: 'auto-tune', disabled: true }],
        // shell 出厂表为空 → 未知 id 拒绝（见拒绝组），此处只验证空域无操作
      },
    ]);
    expect(ids(r.prompt)).toEqual(ids(builtinPromptSections()).filter((id) => id !== 'multi-agent'));
  ...
  it('同层同 id 重复 = 后条覆盖前条', () => {
    const r = resolveRoster(factoryComposition(), [
      {
        prompt: [
          { id: 'memory-brief', disabled: true },
          { id: 'memory-brief', disabled: false },
        ],
      },
    ]);
    expect(ids(r.prompt)).toEqual(ids(builtinPromptSections()));
  ...
  it('text 覆盖：保位 + 保 applicable + render 换固定文本', () => {
    const original = builtinPromptSections();
    const briefIdx = original.findIndex((s) => s.id === 'identity-brief');
    const r = resolveRoster(factoryComposition(), [{ prompt: [{ id: 'identity-brief', text: '整段替换后的文本' }] }]);
    // 保位
    expect(r.prompt.findIndex((s) => s.id === 'identity-brief')).toBe(briefIdx);
    // render 换新文本（无前导分隔符——整段接管，含分隔符由用户文本自备）
    expect(r.prompt[briefIdx].render({ projectPath: '' })).toBe('整段替换后的文本');
    // applicable 保留：identity-brief 的 noGraph 条件在 graphData 存在时返回 false
    expect(r.prompt[briefIdx].applicable?.({ graphData: { nodes: [] }, projectPath: '' })).toBe(false);
    expect(r.prompt[briefIdx].applicable?.({ projectPath: '' })).toBe(true);
    expect(r.diagnostics.overridden).toEqual(['identity-brief']);
  });

  it('text 覆盖两次（跨层）= 后层文本胜出，诊断去重', () => {
    const r = resolveRoster(factoryComposition(), [
      { prompt: [{ id: 'env-brief', text: '第一版' }] },
      { prompt: [{ id: 'env-brief', text: '第二版' }] },
    ]);
    const section = r.prompt.find((s) => s.id === 'env-brief');
    expect(section?.render({ projectPath: '' })).toBe('第二版');
    expect(r.diagnostics.overridden).toEqual(['env-brief']);
  });

  // ── insert 语义 ──

  it('insert after：落在锚段之后', () => {
    const r = resolveRoster(factoryComposition(), [
      {
        prompt: [
          {
            insert: [{ id: 'team-convention', after: 'collaboration-mode', text: '## 团队约定' }],
          },
        ],
      },
    ]);
    ...
    expect(r.prompt[idx - 1].id).toBe('collaboration-mode');
    expect(r.prompt[idx].render({ projectPath: '' })).toBe('## 团队约定');
    expect(r.diagnostics.inserted).toEqual(['team-convention']);
  });

  it('insert before：落在锚段之前', () => {
    const r = resolveRoster(factoryComposition(), [
      {
        prompt: [{ insert: [{ id: 'preface', before: 'behavior-rules', text: '前言' }] }],
      },
    ]);
    const idx = r.prompt.findIndex((s) => s.id === 'preface');
    expect(r.prompt[idx + 1].id).toBe('behavior-rules');
  });
  ...
  it('insert 锚可指禁用行：落位该行原位（终步才过滤）', () => {
    const r = resolveRoster(factoryComposition(), [
      {
        prompt: [
          { id: 'multi-agent', disabled: true },
          { insert: [{ id: 'replacement', after: 'multi-agent', text: '替代段' }] },
        ],
      },
    ]);
    // multi-agent 被禁用移除；replacement 落在 multi-agent 原位
  ...
  it('insert 同层链式：先插段可作后插段锚', () => {
    const r = resolveRoster(factoryComposition(), [
      {
        prompt: [
          {
            insert: [
              { id: 'seg-a', after: 'behavior-rules', text: 'A' },
    ...
    expect(r.prompt[a - 1].id).toBe('behavior-rules');
    expect(b).toBe(a + 1);
  });
  ...
  describe('parseCompositionPatch', () => {
    it('合法全形状（四域 + insert + 覆盖 + 禁用）', () => {
      const raw = {
        ...
        prompt: [
          { id: 'behavior-rules', text: '新规则' },
          { id: 'multi-agent', disabled: true },
          { insert: [{ id: 'extra', after: 'env', text: '补充段' }] },
        ],
