...
const NOTES: Record<string, string> = {
  ...
  // 批 9h-4（2026-09-26）：memory-domain 的管理器 + 记忆束客户端已随包（memory.ts / memory-bundle-client.ts）⇒
...
describe('产物归家账（销账制：搬一个销一条，清空即全绿）', () => {
  it('机械判据自检：真空壳命中、已归家/合格样板不命中（防守卫自身失灵）', () => {
    ...
    expect(
      looksLikeShell('memory-domain').shell,
      'memory-domain 批 9h-4 已归家（包内 memory.ts 管理器 + memory-bundle-client.ts）',
    ).toBe(false);
