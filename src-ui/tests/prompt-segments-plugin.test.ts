...
// 第一方 prompt 段插件（P4 B④ 试点）钉住面：
//   1. 贡献清单：插件装载后注册 memory/claude-md 两段（序 = 迁出前表尾序）；
//   2. 零漂移：通道内缺省拼装（表 11 段 + 贡献 2 段）≡ 迁移前出厂面重述
...
describe('第一方 prompt 段插件（P4 B④ 试点）', () => {
  it('贡献清单：插件装载注册 memory/claude-md（序 = 迁出前表尾序）', async () => {
    const root = new Context();
    const svcFiber = await root.plugin(promptsServicePlugin);
    const segFiber = await root.plugin(promptSegmentsPlugin);
    expect(activePromptContributions().map((s) => s.id)).toEqual(['memory', 'claude-md']);
