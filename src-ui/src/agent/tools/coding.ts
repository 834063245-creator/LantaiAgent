...
// ── 聚合工厂 createCodingTools 已退役（2026-09-24 批 4c-1）──
// 它是「五族拼一起」的便利函数：五族各归其产物包后，内核再拼一次就等于
// **内核 import 产物**（宿主→插件禁反）。消费方只剩测试与 `agent/tool.ts` 的
// 值 re-export（生产走插件通道）；测试改用 `tests/helpers/coding-tools.ts` 的
// `buildCodingTools()` 自行拼接（随各族搬走逐个改指包内）。
