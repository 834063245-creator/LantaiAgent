...
export interface ChatAgentHandle {
  ...
  /** 从**卷日志**恢复折叠状态（压缩摘要 + 尾部起点）。能力位（可选）：句柄不实现 =
   *  无日志面（旧实现/测试桩），调用方降级为「无折叠」而不是炸链路。
   *  为何是重建面：折叠此前是纯运行时态，句柄一重建就丢 ⇒ 载荷回到满值、付过钱的
   *  摘要白丢（2026-09-24 实测：卷 39 压缩后 22,378 → 重启后 295,017）。与账本恢复同规，
   *  开卷 / 惰性补建两条路径都要接。 */
