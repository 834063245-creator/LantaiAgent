 ...
 * ③ **指针悬在交互面上不滚**（按钮/输入件/创作坞/小地图/纸条/文类签/宽度柄/角柄/脊块）——
 *    否则想点按钮、想在输入框打字，画布会自己跑掉；画布上的正文（.pp-block）**不豁免**：
 *    RTS 的镜头就是贴地图边缘走，纸面即地图。**贴边浮件（目次带 / 书脊列）视为画布本体**
 *    （见 HOVER_ALLOW_DOCKS 注：它们不属画布 DOM 却压在左右两条感应带上，不认它 = 那缘
 *    没有边缘滚动）。
 *
...
const HOVER_EXCLUDE = [
  ...
  '.pp-region-edge',
  '.pp-region-corner',
  /* 书脊块（2026-09-21 左缘批）：**瞄准面**——点=定位、拖=落位、hover=出小卡，
   *  三个手势都以「脊不动」为前提。同族的目次带卡片是 `button`（本列已收），
   *  脊块是 `div[role=tab]` ⇒ 不写这一条就漏网（本判据按标签量，不按语义量）。
   *  列**板面与书缝**刻意不在此列（它们算画布，见 HOVER_ALLOW_DOCKS）。 */
  '.sr-spine-main',
  /* 书脊 hover 小卡（架级单卡）：浮在列外、压在画布上的瞄准面——与创作坞/小地图
   *  同族整块豁免（其上的「合卷」是 button，卡身也不是地图）。 */
  '.sr-card',
].join(',');

/** 贴边浮件 = **画布的一部分**（覆盖件宿主不属画布 DOM，但压在画布边缘上）：
 *  目次带是全高、贴在画布最右侧——正盖住右缘那条 36px 感应带（实机：画布
 *  2560 宽、右带 2524–2560）。若不认它，用户把鼠标贴到屏幕最右（真机此处命中的是
 *  `nav.pp-toc` 本身，不是卡片）会被判「不在画布上」⇒ 右缘在用户视角里**根本
 *  没有边缘滚动**（2026-09-17 实机报「右缘不生效」即此）。
 *  故：贴边浮件上的悬停**照滚**，只豁免它上面的交互件——卡片是 `button`（已在
 *  HOVER_EXCLUDE），瞄准卡片时不滚（卡片随世界滚动，一滚就点不中）。
 *  **2026-09-19 加宽批**：带体 64 → 104px（内容列 64 + 缘滚跑道 40），感应带从此
 *  整条落在**跑道**上；内容列（导航面）在带内左 64px，本就在感应带外 ⇒ 用带导航
 *  不再顺带缘滚，而贴屏最右照旧起滚（跑道即为此而立）。跑道上的悬停仍算画布，
 *  只是带内交互（scrub / hover 卡）不认它——判据在 TocStrip 的 `insideCol`。
 *
 *  **2026-09-21 左缘批（用户报「边缘滚动的左侧边缘失灵了，估计是和新的书脊栏逻辑冲突了」）**：
 *  书脊列（`.sr-rack`）同族，但形态相反——它**没有跑道**：列 [0,72] 全高、内容（脊块）
 *  占满 [6,68]，而左缘感应带是 [0,36] ⇒ **整条带压在列内**；列自滚轮批起
 *  `pointer-events: auto`（热区 = 整块板）⇒ 贴屏最左命中的是列板面/脊块，两者都不算
 *  画布 ⇒ 左缘在用户视角里等于没有边缘滚动。
 *  实机取证（`prototype/_probe-edge-left.mjs --stim`；2560×1400 窗、rAF 101fps、
 *  hasFocus=true、带外噪声地板 x=74/400 恒 Δ=0）：x=2/5（列左内距带）、x=20（脊块）、
 *  x=40（题签）各停 500ms → pan **Δ=0**；同一时刻右缘跑道 x=W−2 → Δ=−958、上缘
 *  y=2 → Δ=+1032 ⇒ **只有左缘死**，且死因是「命中列」而不是曲线/带宽。
 *  判据：列的**板面与书缝、最左内距带**算画布（最左那 6px 正是「指针甩到屏左」的
 *  落点——屏缘会把它钉住）；**脊块 / 小卡 / 按钮**是瞄准面，照旧豁免（见 HOVER_EXCLUDE）。
 *  展开态的案卷侧栏（`.ss-sidebar`）**刻意不在列**：它是宽面板（240–420px），悬停其上
 *  照滚 = 上文注 ① 的「跟随相机」，且它一开就整条盖住左缘（此时左缘在用户视角里
 *  不在画布上，同设置面板口径）。 */
const HOVER_ALLOW_DOCKS = '.pp-toc, .sr-rack';
