...
// shell-domain · 宿主依赖面 · 构建产物域（与 host.ts 同形状镜像；类型面以
...
function requireHost(): PluginHostBridge {
  ...
  if (!host) throw new Error('[shell-domain/host.aliased] 宿主桥不可用——内置插件必须在兰台宿主内装载');
  return host;
}
const impl = requireHost().mods.faceDeps as unknown as typeof import('./host');

export const activeShellProviders = impl.activeShellProviders;
export const ownerIdOf = impl.ownerIdOf;
export const ownerSeamView = impl.ownerSeamView;
export const stickyCwdOf = impl.stickyCwdOf;
export const toInputJsonSchema = impl.toInputJsonSchema;

export type ShellAction = import('./host').ShellAction;
