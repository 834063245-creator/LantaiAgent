...
// ask-cards 插件宿主依赖面 · 构建产物域（批 9e-3，2026-09-26）。
...
function requireHost(): PluginHostBridge {
  ...
  if (!host) {
    throw new Error(
      '[ask-cards/host.aliased] 宿主桥不可用——内置插件必须在兰台宿主内装载（window.__lantai_plugin_host__ 缺失）',
    );
  }
  return host;
}
...
export const iconSvg = impl.iconSvg;
export const useCoreStore = impl.useCoreStore;

// 形状面（类型在产物域由内核契约直接给出——契约文件是内核源码，随包内联零状态）
export type {
  AskBatchPrompt,
  AskPrompt,
  AskQuestionItem,
