...
// 动作迁为组合层贡献（对齐 paper-shell 插件先例——「重构推到哪个域，
// 行化跟到哪个域」）。settings.ts 持久化与 agent-config-store 订阅
...
// 产物域（dist-plugins/builtin/hologram/settings-domain/，manifest 声明
// displace 位移 bundle 行；设置面板「重新加载」热替换）。
