...
// asset-DomainPlugin 域插件宿主依赖面 · 构建产物域（增补四）。与 host.ts 同形状，从宿主桥
...
function requireHost(): PluginHostBridge {
  ...
  if (!host) {
    throw new Error(
      '[asset-DomainPlugin/host.aliased] 宿主桥不可用——内置插件必须在兰台宿主内装载（window.__lantai_plugin_host__ 缺失）',
    );
  }
  return host;
}
...
export const assetDomainPlugin = impl.assetDomainPlugin;
