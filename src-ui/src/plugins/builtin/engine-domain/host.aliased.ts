...
function requireHost(): PluginHostBridge {
  ...
  if (!host) throw new Error('[engine-domain/host.aliased] 宿主桥不可用——内置插件必须在兰台宿主内装载');
  return host;
}
const impl = requireHost().mods.faceDeps as unknown as typeof import('./host');
export const z = impl.z;
export const defineTool = impl.defineTool;
export const loadHologramSchemas = impl.loadHologramSchemas;
export const mcpSchemaToTool = impl.mcpSchemaToTool;
export const graphExecute = impl.graphExecute;
export const agentInvoke = impl.agentInvoke;
