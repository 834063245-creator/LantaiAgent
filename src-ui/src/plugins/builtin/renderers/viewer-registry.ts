...
export interface ViewerProps {
  ...
  label: string;
  filePath?: string;
  /** needsBytes 且未超限时给；缺省 = 该查看器不看字节（如归档只列名、旧 Office 只出壳） */
  bytes?: ViewerBytes;
  mode: ViewerMode;
...
export interface ViewerDef {
  /** 机器名（'image' / 'video' / 'audio' / 'pdf' …）——重名**装载期拒绝** */
  id: string;
  /** 认领的扩展名（小写无点；装载期归一；跨查看器重名**装载期拒绝**） */
  exts: readonly string[];
  /** ext → MIME（needsBytes 时每个 ext 必填——缺了宿主拼不出 data URI，装载期拒绝） */
  mimes?: Readonly<Record<string, string>>;
  /** 要不要读文件字节；false = 只看路径与元数据 */
  needsBytes: boolean;
  /** 字节上限（超出 → 文件壳 + 可读错误，**不静默截断**）；缺省 = 不设限 */
  maxBytes?: number;
...
class ViewerRegistry {
  ...
  register(def: ViewerDef): () => void {
    ...
    for (const ext of exts) {
      ...
      if (def.needsBytes && !def.mimes?.[ext]) {
        throw new Error(
