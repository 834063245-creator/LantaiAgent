...
// git / search 域第一方插件（P4 存量拆解 B①，agent-plugin-architecture-plan §5）——
// 两族从 composition/tool-rows 行表迁入 ctx.tools 贡献通道（域插件形状对齐
// S3 settings-plugin 先例：一域一插件，disposer 经 ctx.effect 登记）。
//
// 搬运勘定（baton7 §1）：两族只依赖 codingExec——装配期新闭包但语义无状态
// （闭包只引用模块级 agentInvoke/execStreamedShell，不捕获装配 opts），因此
// pluginToolRows 的实例缓存（首装配实例跨装配复用）语义等价。依赖装配期
// 真值的族（ask 的 ui 回调 / wait 的 subAgentPool）不适用本通道，仍留行表。
//
// 可见面零漂移按构造成立：git/search 旧名在 convergeRegistry 后经
// collectHiddenToolNames 隐藏，可见域工具由 DOMAIN_SPECS 声明序重建——
// 走行表还是插件通道注册不改变模型可见面（phase-0 快照守护）。
//
// 一文件两插件：familyContributions 族贡献清单 helper 由两域共享（族注册
// 形状相同，拆文件只会复制 helper）。

import type { Tool, ToolExecutor } from '../agent/tool';
import { createGitTools, createSearchTools } from '../agent/tools/coding';
...
const NEVER_EXEC: ToolExecutor = async () => {
  throw new Error('[git-search-plugin] apply 期占位 exec 不应被调用');
};
