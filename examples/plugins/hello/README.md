...
> 装 → 用 → 卸全链路走查通过——安装后 Hello 面板上轨道并渲染卡片、
> Ctrl+K 搜「Hello」命令可见且点击弹状态栏通知、`hello_greet` 工具行
> 折算进装配面（`plugin/hello/greet`）、设置「插件」tab 列出 hello
> （运行中徽章 + 供应链警告常驻）、UI 点「卸载」后重启面板/命令/工具
> 三通道干净退出。热重载与 preset 切换同场验证（改 patch 6s 内生效、
> 删除显式回退出厂、minimal 选择持久化重启保持）。
